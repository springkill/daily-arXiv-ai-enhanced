...
界面语言存在 `localStorage` 的 `arxiv_lang`,没选过就弹一次首次选择,之后在
「设置 → 语言」里随时改。词条集中在 `js/i18n.js`,取词用 `I18n.t('key', {vars})`。

三处影响、三种生效方式:

| 影响 | 生效 | 为什么 |
|---|---|---|
| 界面文案 | 立即 | 纯前端。静态 HTML 用 `data-i18n` / `data-i18n-attr` 标记,`applyStatic()` 一次性替换;动态部分由各视图的 `rerender()` 重画 |
| 审稿意见 | 下一次审稿 | 前端把当前语言随请求发给后端(白名单枚举),写进 prompt。已存的结果按当时语言保留,不追溯翻译 |
| 论文总结 | 改不了 | 由流水线在生成时决定(`.env.local` 的 `LANGUAGE`)。前端只能在当天实际产出的语言里挑,挑不到会在工具行提示 |

### 两个实现上的坑

**不要导出全局 `t`。** `compromise.js`(统计页做名词短语抽取用的)也占用全局 `t`,
而且它在 `js/i18n.js` 之后加载,会把我们的覆盖掉 —— 表现是界面上直接显示出一段
JS 源码。各模块在 IIFE 顶部写 `var t = I18n.t;`,并且注意别用 `t` 当回调形参。

**首屏要主动调一次 `applyStatic()`。** `js/i18n.js` 在 `<head>` 里执行时 DOM 还没建好,
只设 `<html lang>` 是不够的,得等 `DOMContentLoaded` 再刷一遍静态文案。

### 审稿结论存的是枚举

`recommend` 在库里存 `accept / minor / major / reject`,不是中文。存本地化文本的话,
用户切到另一种语言就会看到一半是旧语言,而且没法按结论排序筛选。prompt 里明确要求
这个字段输出英文枚举、其余正文用目标语言。老库里的中文值由 `store.migrate()`
自动转换(列和 `sections` JSON 两处都转)。

### 加一种语言

在 `js/i18n.js` 的 `DICT` 里加一份词条 + `CATEGORIES` 里加类别名 + `LANGS` 里加一项;
后端 `deploy/api/server.py` 的 `LANGS` 白名单里加同名条目。缺词条时 `t()` 回落到英文,
再缺就显示 key 本身,方便一眼看出漏翻。
