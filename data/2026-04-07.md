...
TL;DR: LLM与人类在群体协调游戏中的表现对比：人类能适应并稳定行为，而LLM常失败且过度切换，影响群体收敛


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否具备与人类相当的适应性协调能力，以及它们是否使用与人类相同的策略

Method: 使用群体二分搜索游戏（Group Binary Search）比较LLM和人类表现，这是一个具有不完美监控的共同利益游戏，参与者需要协调行动以达到共同目标

Result: 人类能适应并稳定行为，而LLM经常无法改进，表现出过度切换，影响群体收敛；更丰富的反馈对人类有益但对LLM影响小

Conclusion: 通过基于人类基线和机制层面指标的分析，指出了人类和LLM群体之间的差异，并为缩小协调差距提供了行为基础诊断
...
TL;DR: 该论文首次对LLM多智能体系统中的协调动力学进行了大规模实证研究，揭示了协调遵循重尾级联、通过优先连接集中到知识精英、以及随着系统规模增长产生更频繁极端事件的三条耦合定律，并提出集成瓶颈机制和DTI解决方案。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统作为交互智能体社会被广泛部署，但系统扩展常导致收益递减或不稳定，其原因尚不清楚。需要理解多智能体系统中的协调动力学，以改进可扩展的集体智能。

Method: 引入原子事件级表述，将推理重构为协调级联。分析超过150万次跨任务、拓扑和规模的交互。提出集成瓶颈机制，并引入赤字触发集成(DTI)方法，在失衡时选择性增加集成。

Result: 发现三条耦合定律：协调遵循重尾级联分布；通过优先连接集中到知识精英；系统规模增长导致极端事件更频繁。验证了集成瓶颈机制，DTI在协调失败时能提高性能而不抑制大规模推理。

Conclusion: 研究建立了集体认知的定量定律，识别协调结构作为理解和改进可扩展多智能体智能的基本、先前未测量的维度。集成瓶颈机制和DTI为解决多智能体系统扩展问题提供了理论基础和方法。
...
TL;DR: 提出基于多智能体强化学习的微电网社区日内P2P交易框架，平衡个体经济利益与社区低碳排放目标


<details>
  <summary>Details</summary>
Motivation: 可再生能源发电的不确定性和实时市场的不稳定性限制了微电网社区中清洁能源的有效利用。现有的P2P和微电网协调方法通常依赖集中式优化或限制性协调规则，难以在实际应用中实施。

Method: 将微电网决策过程建模为分散式部分可观测马尔可夫决策过程，采用多智能体强化学习框架求解。引入新型市场出清机制进行宏观调节，激励微电网优先消纳本地可再生能源。

Result: 仿真结果表明，自利投标策略与P2P市场设计的结合显著提高了可再生能源利用率，减少了对高碳排放外部电力的依赖。框架实现了本地自治、自利追求与社区层面经济环境效益的平衡整合。

Conclusion: 该框架为微电网社区提供了一种可行的日内P2P交易解决方案，在保障个体自主决策的同时，通过市场机制引导实现社区低碳排放目标，促进了可再生能源的有效利用。
...
### [4] [A Logic of Secrecy on Simplicial Models](https://arxiv.org/abs/2604.02673)
*Shanxia Wang*

Main category: cs.LO

TL;DR: 该论文提出了一种基于单纯形模型的保密逻辑，通过在标准单纯形认知模型中添加保密邻域函数来形式化保密概念，并建立了相应的公理系统SSL。


<details>
  <summary>Details</summary>
Motivation: 标准单纯形模型为多智能体系统提供了几何语义，通过染色单纯复形表示全局状态，彩色顶点表示智能体局部状态。然而，仅依靠普通单纯形知识结构无法将保密性作为真正的新模态来捕捉，这促使引入额外的保密层。

Method: 定义了单纯形保密模型，在标准单纯形认知模型基础上增加了与局部状态关联的智能体相对保密邻域函数。引入原始保密算子S_aφ，其语义要求：智能体a在普通单纯形意义下知道φ，且φ的真值集属于与a当前局部状态关联的指定保密邻域之一。构建了公理系统SSL，并针对|A|≥2的多智能体情况，通过构造辅助颜色典范模型并在原始纯A-染色单纯形保密模型类中表示它来证明完备性。

Result: 证明了SSL相对于单纯形保密模型类是可靠的。对于真正多智能体情况(|A|≥2)，证明了系统的完备性。该框架为单纯形模型上的保密性提供了原始、基于局部状态、几何基础的形式化描述，并给出了可靠的公理化系统，在多智能体情况下是完备的。

Conclusion: 该研究成功地在单纯形模型中引入了保密逻辑，通过添加保密邻域函数扩展了标准单纯形认知模型，建立了相应的公理系统并证明了可靠性和完备性，为多智能体系统中的保密性分析提供了几何基础的形式化框架。
...
### [5] [Compositionality of Lyapunov functions via assume-guarantee reasoning](https://arxiv.org/abs/2604.03017)
*Matteo Capucci,David Jaz Myers*

Main category: cs.LO

TL;DR: 提出一个基于范畴论的安全问题假设-保证推理框架，将系统视为透镜，适用于广义摩尔机及其变体（包括控制系统的输入-状态稳定性分析）。


<details>
  <summary>Details</summary>
Motivation: 为组合模型检查中的假设-保证推理提供统一的范畴论框架，特别针对广义摩尔机及其变体（包括部分可观测马尔可夫过程、参数化ODE系统等），实现形式化且可组合的推理方法。

Method: 将系统建模为透镜，基于切性（具有截面的纤维化）定义广义摩尔机。通过对称幺半松散右模构造假设-保证认证的广义摩尔机，并在认证接线图的双范畴中建立2函子性框架。

Result: 建立了范畴自然的组合式假设-保证推理框架，特别为参数化ODE系统的（局部）输入-状态稳定性李雅普诺夫函数提供了新颖的假设-保证推理公式。

Conclusion: 该范畴论框架为广义摩尔机家族（包括控制系统的稳定性分析）提供了统一、可组合的假设-保证推理方法，扩展了组合模型检查的形式化工具。
...
### [6] [HistMSO: A Logic for Reasoning about Consistency Models with MONA](https://arxiv.org/abs/2604.03085)
*Isabelle Coget,Étienne Lozes*

Main category: cs.LO

TL;DR: 提出HistMSO逻辑用于复制数据系统一致性模型的形式化推理，能够表达42种一致性模型中的39种，并开发了将HistMSO可满足性和模型检测归约到字上MSO问题的方法，利用MONA工具实现自动化推理。


<details>
  <summary>Details</summary>
Motivation: 复制数据系统的一致性模型推理具有挑战性，需要深入理解一致性模型本身，且在机械化验证方法中需要大量人工输入。现有方法在形式化推理和自动化验证方面存在不足。

Method: 引入HistMSO（历史和抽象执行的单子二阶逻辑），基于Burckhardt提出的复制数据系统执行形式模型。开发了将HistMSO可满足性和模型检测问题归约到字上MSO问题的方法，并利用MONA工具实现自动化推理。

Result: HistMSO能够表达Viotti和Vukolic层次结构中42种一致性模型中的39种。通过归约到字上MSO问题，实现了对一致性模型的自动化推理和验证。

Conclusion: HistMSO为复制数据系统一致性模型提供了强大的形式化推理框架，能够表达绝大多数现有一致性模型，并通过归约到字上MSO实现了自动化验证，显著降低了人工验证负担。
...
### [7] [Computing with Living Neurons: Chaos-Controlled Reservoir Computing with Knowledge Transplant](https://arxiv.org/abs/2604.02552)
*Seung Hyun Kim,Zhi Dou,Gaurav Upadhyay,Anay Pattanaik,Leo Maslov,Lav Varshney,John Beggs,Howard Gritton,Mattia Gazzola*

Main category: cs.NE

TL;DR: 提出混沌控制储备池计算(cc-RC)用于活体神经培养物，结合预训练识别、低功率光学混沌控制和受控状态下的读出训练，显著提升学习性能；进一步提出知识移植(KT)方法，将专家培养物的储备映射移植到等效学生培养物，实现跨底物的可重用学习模型。


<details>
  <summary>Details</summary>
Motivation: 活体神经培养物作为动态丰富的计算底物具有独特潜力，但存在固有的生物变异性问题。需要开发能够适应这种变异性并实现稳健学习的方法，同时探索如何跨神经群体共享和积累知识，超越生物寿命限制。

Method: cc-RC方法包含三个步骤：(1)预训练识别每个培养物的动态特征和相空间吸引子；(2)低功率光学混沌控制以稳定自发和刺激诱发活动；(3)在受控状态下训练读出层。进一步提出知识移植(KT)，将专家培养物学习到的储备映射移植到吸引子等效的学生培养物。

Result: 在数百个神经样本上，cc-RC实现了稳健的学习和模式分类，相比标准RC将准确率和模型寿命提高了约300%。知识移植(KT)将训练时间缩短至分钟级别，同时提升性能，实现了跨底物的可重用学习模型。

Conclusion: cc-RC为活体神经培养物的稳健计算提供了有效框架，而KT方法通过实现跨神经群体的知识积累和共享，超越了生物寿命限制，为神经计算系统的发展开辟了新途径。
...
### [8] [Frame Theoretical Derivation of Three Factor Learning Rule for Oja's Subspace Rule](https://arxiv.org/abs/2604.02849)
*Taiki Yamada*

Main category: cs.NE

TL;DR: 从框架理论角度系统推导了EGHR-PCA学习规则，证明其可从Oja子空间规则导出，全局第三因子对应对称矩阵空间自然框架的框架系数


<details>
  <summary>Details</summary>
Motivation: 为生物合理的学习规则（EGHR-PCA）提供原则性、非启发式的数学推导，将其与数学规范对应物（Oja子空间规则）建立联系

Method: 使用框架理论，将Oja子空间规则在对称矩阵空间的自然框架上展开，系统推导出EGHR-PCA三因子学习规则

Result: 证明EGHR-PCA中的全局第三因子精确对应框架展开中的框架系数，在Gaussian输入下EGHR-PCA等价于Oja子空间规则

Conclusion: 通过框架理论为生物合理学习规则提供了严格的数学基础，建立了生物可塑性规则与规范数学形式之间的直接联系
...
### [9] [Accelerating Black-Box Bilevel Optimization with Rank-Based Upper-Level Value Function Approximation](https://arxiv.org/abs/2604.02888)
*Marc Ong,Youhei Akimoto*

Main category: cs.NE

TL;DR: 提出基于排序不变性的进化算法框架，通过近似上层目标函数排序来避免下层优化收敛，显著降低双层优化计算成本


<details>
  <summary>Details</summary>
Motivation: 传统进化方法处理双层优化问题计算成本高，现有启发式方法在复杂优化场景（如多模态、变量强交互）中适应性有限

Method: 利用基于排序的进化算法对单调变换的不变性，直接近似上层目标函数排序，绕过下层优化收敛需求，采用CMA-ES作为优化器

Result: 在标准基准测试中表现优异，能够解决传统方法难以处理的多模态和强变量交互问题

Conclusion: 提出的框架通过利用排序不变性有效降低了双层优化计算负担，在复杂优化场景中展现出强大性能
...
### [10] [Red Flags and Cherry Picking: Reading The Scientific Blackpill Wiki](https://arxiv.org/abs/2604.02565)
...
TL;DR: 该研究分析了"科学黑丸"维基页面，发现incel社区引用合法科学研究但过度概括、剥离语境或扭曲结果来支持其既有意识形态，这与其他错误信息和阴谋论意识形态借用科学合法性的模式相似。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析incel社区如何利用科学文献来支持其极端厌女、暴力美化、生物本质主义的意识形态，特别是评估他们如何正确引用或滥用科学研究来论证其"黑丸"信仰体系。

Method: 研究方法是对"科学黑丸"维基页面上的主张进行系统分析，将页面引用的研究与其原始科学文献进行对比，评估维基作者如何准确描述或扭曲科学发现来支持incel意识形态。

Result: 研究发现该页面主要引用合法的科学研究，并且部分或大部分描述是准确的。然而，在讨论这些研究时，结果经常被过度概括、剥离语境或以其他方式扭曲，以支持incel既有的观点。这与先前关于动机性推理和其他错误信息、阴谋论意识形态借用科学合法性的发现相呼应。

Conclusion: 结论是incel社区通过选择性使用和扭曲科学文献来为其意识形态提供表面上的科学合法性，这种模式反映了在线激进化过程中信息质量的挑战，以及科学如何被滥用来支持极端信仰体系。
...
### [11] [Making Written Theorems Explorable by Grounding Them in Formal Representations](https://arxiv.org/abs/2604.02598)
...
<details>
  <summary>Details</summary>
Motivation: LLM生成的解释虽然能提高技术内容可访问性，但存在交互性上限。由于LLM输出是静态文本，无法执行或逐步调试。作者认为将解释基于形式化表示能实现超越静态文本的交互功能

Method: 开发"可探索定理"系统，使用LLM将定理及其书面证明转换为Lean（机器检查证明的编程语言），并将书面证明与Lean代码链接。读者可以逐步执行证明、测试自定义示例或反例、追踪逻辑依赖关系

Result: 用户研究（n=16）显示该方法具有潜在优势：在证明阅读任务中，能够使用可探索功能的参与者对理解问题给出了更好、更正确、更详细的答案，表现出对底层数学更强的整体理解

Conclusion: 将LLM生成解释与形式化表示相结合，通过可探索定理系统实现了超越静态文本的交互式数学证明理解，显著提升了学习效果
...
### [12] [LitPivot: Developing Well-Situated Research Ideas Through Dynamic Contextualization and Critique within the Literature Landscape](https://arxiv.org/abs/2604.02600)
*Hita Kambhamettu,Bhavana Dalvi Mishra,Andrew Head,Jonathan Bragg,Aakanksha Naik,Joseph Chee Chang,Pao Siangliulue*

Main category: cs.HC

TL;DR: LitPivot是一个支持研究想法与文献动态交互的工具，通过文献引导的转向机制，帮助研究人员在构思过程中根据相关文献动态调整研究想法，同时根据想法变化重新检索相关文献。


<details>
  <summary>Details</summary>
Motivation: 现有工具存在局限性：文献工具仅帮助理解固定文献集，而构思工具则基于静态预选文献集评估想法。缺乏支持想法与文献动态交互的工具，无法应对研究过程中想法变化导致相关文献变化的情况。

Method: 提出文献引导的转向机制，并在LitPivot中实现。研究人员可同时起草和验证想法，系统动态检索与想法特定部分相关的论文簇，并提供基于文献的批判性建议以指导想法修订。

Result: 实验室研究（n=17）显示，研究人员使用LitPivot产生了评分更高的想法，并自我报告对文献空间有更强的理解；开放式研究（n=5）揭示了研究人员如何使用LitPivot迭代演化自己的想法。

Conclusion: LitPivot通过支持想法与文献的动态交互，有效解决了研究构思过程中的关键挑战，帮助研究人员产生更高质量的研究想法并加深对文献领域的理解。
...
### [13] [Toys that listen, talk, and play: Understanding Children's Sensemaking and Interactions with AI Toys](https://arxiv.org/abs/2604.02629)
*Aayushi Dangol,Meghna Gupta,Daeun Yoo,Robert Wolfe,Jason Yip,Franziska Roesner,Julie A. Kientz*

Main category: cs.HC

TL;DR: 儿童与AI玩具互动研究：通过参与式设计探究儿童如何理解AI玩具的边界、能动性和关系，发现儿童将AI玩具视为社交实体，但交互故障导致对抗性游戏行为。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正越来越多地融入儿童的日常生活，不仅通过屏幕，还通过所谓的"无屏幕"AI玩具。这些玩具能够模拟情感、个性化回应并回忆先前的互动，创造出持续社交连接的假象。这种能力引发了重要问题：儿童在与AI玩具互动时如何理解边界、能动性和关系。

Method: 进行了两次参与式设计会议，涉及8名6-11岁的儿童。他们与三种不同的AI玩具互动，在游戏、实验和反思之间切换，观察儿童如何理解和应对这些AI玩具。

Result: 研究发现儿童以真诚的好奇心接近AI玩具，将其视为社交实体。然而，频繁的交互故障以及明显智能与玩具形态之间的不匹配破坏了游戏期望，导致了对抗性游戏行为。

Conclusion: 提出了启示和设计建议，旨在以更透明、发展适宜和负责任的方式引导儿童与AI玩具的相遇，强调需要更好地设计AI玩具以支持儿童的健康发展。
...
### [14] [The Paradox of Prioritization in Public Sector Algorithms](https://arxiv.org/abs/2604.02641)
*Erina Seh-Young Moon,Shion Guha*

Main category: cs.HC

TL;DR: 该研究揭示公共部门采用算法优先级排序工具时，随着资源日益稀缺，优先级排序机制会在交叉身份群体间产生显著相对差异，加剧不平等感知，并警示不应将算法效率与"用更少资源做更多事"的修辞混为一谈。


<details>
  <summary>Details</summary>
Motivation: 公共部门越来越多地采用算法优先级排序工具来分配稀缺公共资源，现有研究主要关注公平性、准确性和有效性评估，但优先级排序本身的结构设计如何影响工具效果及受影响个体的体验，在真实公共部门条件下仍缺乏研究。

Method: 研究通过分析优先级排序的底层机制，展示随着资源稀缺性增加，这些机制如何在交叉身份群体间产生显著相对差异，并论证优先级排序会加剧受影响个体的不平等感知。

Result: 研究表明优先级排序方法在公共部门存在缺陷：随着资源日益稀缺，优先级排序机制会在交叉身份群体间产生显著相对差异，加剧不平等感知，且算法工具产生的效率不应与"用更少资源做更多事"的修辞混为一谈。

Conclusion: 公共部门采用算法优先级排序工具需要谨慎，优先级排序本身会加剧群体间不平等，算法效率不应被误解为"用更少资源做更多事"，必须关注现实实施环境中的资源约束风险。
...
### [15] [Engagement Is Not Transfer: A Withdrawal Study of a Consumer Social Robot with Autistic Children at Home](https://arxiv.org/abs/2604.02642)
*Yibo Meng,Guangrui Fan,Bingyi Liu,Yingfangzhong Sun,Ruiqi Chen,Haipeng Mi*

Main category: cs.HC

TL;DR: 社交机器人干预对自闭症儿童的影响：持续接触降低焦虑但阻碍社交能力向人类转移，而撤回机器人反而促进社交动机和情感理解发展


<details>
  <summary>Details</summary>
Motivation: 研究社交机器人参与度是否能够转化为自闭症儿童对人类社交能力的改善，探索机器人干预对社交转移效果的影响

Method: 8周家庭随机对照试验，40名5-9岁自闭症儿童使用商业社交机器人Qrobot，分为持续接触组和撤回组，通过定量测量和照顾者访谈评估焦虑、社交动机、情绪推理和共情

Result: 持续接触组显著降低焦虑，确认强烈情感效益和高可用性；撤回组在社交动机、情绪理解和共情行为方面改善更大，表现出"交接vs隔离"模式：撤回促进重新定向人类社交，持续接触限制向现实世界转移

Conclusion: 高参与度不能保证社交转移，机器人撤回可能通过促进重新定向人类社交互动而带来更大社交效益，需要平衡机器人参与和人类社交转移
...
### [16] [Beyond the AI Tutor: Social Learning with LLM Agents](https://arxiv.org/abs/2604.02677)
*Harsh Kumar,Zi Kang,Mu,Jonathan Vincentius,Ashton Anderson*

Main category: cs.HC

TL;DR: 多智能体LLM配置相比单一LLM导师能提升学习效果：在数学问题解决中，同时接触导师和同伴（犯不同类型错误）的参与者表现最佳；在写作任务中，双智能体配置避免了单一模型导致的思维同质化。


<details>
  <summary>Details</summary>
Motivation: 当前AI教育工具主要采用一对一辅导模式，但学习科学研究表明多主体互动（同伴建模、共同建构、接触多元视角）能带来一对一辅导无法实现的学习效益。研究旨在探索多智能体LLM配置是否能超越单一LLM导师的学习效果。

Method: 采用两个对照实验：1) 收敛性问题解决研究(N=315)，参与者解决SAT级别数学问题，采用2×2设计（有无LLM导师×有无LLM同伴，同伴犯不同类型错误）；2) 发散性写作研究(N=247)，参与者撰写议论文和创意文章，比较无AI辅助、单一LLM(Claude或ChatGPT)、双LLM(Claude+ChatGPT)三种条件。

Result: 数学问题解决中，同时与导师和同伴互动的参与者获得最高的独立测试准确率；写作任务中，两种LLM条件都提升了文章质量，但只有双智能体配置避免了单一模型辅助导致的思维同质化问题。

Conclusion: 多智能体LLM学习环境能提供比单一LLM导师更好的学习效果，从一对一AI辅导转向更丰富的智能体配置可以解锁人类社交学习研究中长期记录的协作和观察效益。
...
### [17] [AI Disclosure with DAISY](https://arxiv.org/abs/2604.02760)
*Yoana Ahmetoglu,Marios Constantinides,Anna Cox*

Main category: cs.HC

TL;DR: DAISY是一个基于表单的工具，用于生成AI使用披露声明，通过结构化披露提高报告完整性和透明度，同时不降低作者对披露声明的舒适度。


<details>
  <summary>Details</summary>
Motivation: AI工具在研究中的使用日益普遍，但AI披露声明仍然罕见且不一致。现有政策指导有限，作者在报告AI使用时面临社会、认知和情感障碍，需要探索结构化披露如何影响作者报告内容和披露体验。

Method: 开发DAISY（Disclosure of AI-uSe in Your Research）表单工具，基于文献需求分析和共同设计（N=11），并在作者用户研究中进行部署（N=31），比较DAISY支持的披露与非结构化披露的差异。

Result: DAISY支持的披露满足更多完整性标准，在研究过程和写作中提供更清晰的AI使用细分。令人惊讶的是，尽管作者担心透明披露AI使用可能带来的负面看法，但使用DAISY并未降低他们对披露声明的舒适度。

Conclusion: 研究讨论了AI披露作为社会技术实践的设计意义和研究议程，表明结构化工具如DAISY可以改善AI使用报告的完整性和透明度，同时维持作者的心理舒适度。
...
### [18] [Disrupting Cognitive Passivity: Rethinking AI-Assisted Data Literacy through Cognitive Alignment](https://arxiv.org/abs/2604.02783)
*Yongsu Ahn,Nam Wook Kim,Benjamin Bach*

Main category: cs.HC

TL;DR: 论文提出认知对齐框架，通过匹配用户认知需求与AI交互模式来避免认知被动性，促进数据素养发展


<details>
  <summary>Details</summary>
Motivation: AI助手默认的全面一次性响应模式可能导致用户认知被动性，削弱数据素养发展机会，需要更动态的交互策略

Method: 提出认知对齐框架，将AI交互模式（传递式或审慎式）与用户认知需求（接受式或审慎式）进行匹配，避免认知被动或摩擦

Result: 建立了有效人机交互作为用户认知需求与AI交互模式对齐的函数框架，提供了具体映射关系

Conclusion: 需要动态自适应策略而非简单让AI促进审慎思考，认知对齐框架为未来数据素养研究提供理论基础和开放问题
...
### [19] [UnrealVis: A Testing Laboratory of Optimization Techniques in Unreal Engine for Scientific Visualization](https://arxiv.org/abs/2604.02980)
*Matteo Filosa,Andrea Nardocci,Tiziana Catarci,Marco Angelini*

Main category: cs.HC

TL;DR: UnrealVis是一个基于Unreal Engine的优化实验室，用于在交互式探索中配置和评估渲染技术，帮助用户在性能和保真度之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 可视化大型3D科学数据集需要在性能和保真度之间取得平衡，但传统工具通常需要过多的技术专业知识，这限制了科学家的使用。

Method: 通过回顾55篇论文，建立了包含22种优化技术的分类法，涵盖六个技术家族，并通过Unreal Engine子系统（如Nanite、LOD方案和剔除技术）实现。系统提供直观的工作流程，包含实时遥测和A/B比较功能，用于本地和全局性能分析。

Result: 通过核糖体结构和体积流场的案例研究以及专家评估验证，UnrealVis能够帮助用户选择满足性能目标同时保持结构保真度的优化组合。

Conclusion: UnrealVis为科学可视化提供了一种系统化的优化方法，降低了技术门槛，使科学家能够更有效地在交互式探索中平衡渲染性能和视觉保真度。
...
### [20] [MECO: A Multimodal Dataset for Emotion and Cognitive Understanding in Older Adults](https://arxiv.org/abs/2604.03050)
*Hongbin Chen,Jie Li,Wei Wang,Siyang Song,Xiao Gu,Jianqing Li,Wentao Xiang*

Main category: cs.HC

TL;DR: MECO是一个针对老年人群的多模态情感与认知数据集，包含视频、音频、EEG和ECG信号，旨在填补老龄化人群情感计算研究的空白。


<details>
  <summary>Details</summary>
Motivation: 现有情感计算研究主要针对年轻、认知健康的受试者，缺乏专门针对老龄化人群的数据集，而认知衰退会影响情感表达和生理反应，因此需要专门的数据资源来研究老年人群的情感与认知。

Method: 收集了42名参与者的多模态数据，包括视频、音频、EEG和ECG信号，采用标准化协议在社区环境中采集，确保生态效度。数据集包含30,592个同步样本，约38小时数据，并提供了情感状态（效价、唤醒度、六种基本情绪）和认知状态（MMSE评分）的全面标注。

Result: 建立了情感和认知预测的基准测试，MECO数据集可作为研究老龄化人群情感与认知多模态建模的基础资源，支持个性化情感识别和轻度认知障碍早期检测等下游应用。

Conclusion: MECO填补了老龄化人群多模态情感计算研究的空白，为理解认知衰退对情感表达的影响提供了重要数据支持，有助于开发针对老年人群的情感识别和认知健康监测系统。
...
### [21] [Same Feedback, Different Source: How AI vs. Human Feedback Attribution and Credibility Shape Learner Behavior in Computing Education](https://arxiv.org/abs/2604.03075)
*Caitlin Morris,Pattie Maes*

Main category: cs.HC

TL;DR: 研究探讨学习者对反馈来源的认知是否影响学习效果，通过三组实验（AI即时反馈、AI延迟反馈、人类延迟反馈）分离来源归因与时间因素，发现人类归因能增加学习时间但需要可信度支撑。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地承担教学角色（提供反馈、指导练习、评估作业），一个基本问题出现：学习者认为反馈来自谁是否重要？先前研究未能控制交付时间这一混淆变量，本研究旨在分离来源归因与时间因素的影响。

Method: 采用三条件实验设计（N=148）：参与者完成创意编码教程，接收由同一大型语言模型生成的反馈，分别归因于AI系统（即时或延迟交付）或人类助教（匹配延迟交付）。这种设计分离了来源归因与交付时间的影响。探索性分析还考察了参与者对归因可信度的认知。

Result: 来源归因和时间因素对不同结果有不同影响：认为反馈来自人类的参与者比接收同等时间AI归因反馈的参与者花费更多时间（d=0.61, p=.013）；交付延迟独立增加了输出复杂度但不影响时间测量。探索性分析显示，46%的人类归因组参与者不相信归因，这些参与者比接收透明AI反馈的参与者表现更差（代码复杂度d=0.77, p=.003；任务时间d=0.70, p=.007）。

Conclusion: 学习者认为的人类存在可能具有激励价值，但这种价值取决于可信度。对于计算教育者，在人类归因不可信的背景下，透明的AI归因可能是风险较低的默认选择。研究强调了分离归因与时间因素的重要性，并指出可信度是影响学习效果的关键中介变量。
...
### [22] [CASCADE: A Cascading Architecture for Social Coordination with Controllable Emergence at Low Cost](https://arxiv.org/abs/2604.03091)
*Yizhi Xu*

Main category: cs.HC

TL;DR: CASCADE是一个三层架构，用于在沙盒游戏中实现低成本、可控的社会协调，通过宏观状态管理、模块化协调中心和标签驱动NPC来平衡创作控制与计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有脚本NPC系统虽然高效但僵化，而完全LLM驱动的智能体虽然能产生更丰富的社会行为但运行时成本过高。需要一种既能保持创作控制又能降低计算成本的解决方案。

Method: 提出CASCADE三层架构：1) 宏观状态导向器维护离散时间世界状态变量和宏观因果更新；2) 模块化协调中心通过领域特定组件分解状态变化，并将指令路由到标签定义组；3) 标签驱动NPC通过行为树和本地状态/效用函数执行响应，仅在面向玩家的交互中按需调用大语言模型。

Result: 通过多个微场景原型和基于轨迹的分析评估，显示共享宏观事件可以产生差异化但逻辑受限的NPC行为，而无需在主模拟循环中进行每个智能体的提示。CASCADE为可扩展的社会模拟和未来开放世界创作工具提供了模块化基础。

Conclusion: CASCADE提供了一个平衡创作控制与计算成本的解决方案，通过分层架构实现了低成本、可控的社会协调，为可扩展的游戏社会模拟提供了有效框架。
...
### [23] [A Dynamic Toolkit for Transmission Characteristics of Precision Reducers with Explicit Contact Geometry](https://arxiv.org/abs/2604.02387)
*Jiacheng Miao,Chao Liu,Qiliang Wang,Yunhui Guan,Weidong He*

Main category: cs.RO

TL;DR: 提出用于精密减速器传动特性分析的动态工具包，采用显式接触几何建模，提供更高精度和计算效率的模块化解决方案


<details>
  <summary>Details</summary>
Motivation: 精密减速器是机器人系统的关键部件，直接影响各类机器人的运动精度和动态性能。现有方法在精确接触行为建模、齿轮刚度评估和系统振动预测方面存在挑战，需要更精确高效的分析工具。

Method: 提出统一框架，集成先进接触理论和数值求解方法，采用显式接触几何建模。工具包采用模块化、可脚本化架构，支持跨不同减速器拓扑的快速重构。

Result: 与现有动力学软件相比，所提工具包提供更高的精度和计算效率。通过已发布基准的数值验证，确认了该方法的准确性。

Conclusion: 该动态工具包为精密减速器的传动特性分析提供了有效的解决方案，能够更精确地建模接触行为、评估刚度和预测振动，适用于各类机器人系统的减速器设计优化。
...
### [24] [Backup-Based Safety Filters: A Comparative Review of Backup CBF, Model Predictive Shielding, and gatekeeper](https://arxiv.org/abs/2604.02401)
*Taekyung Kim,Aswin D. Menon,Akshunn Trivedi,Dimitra Panagou*

Main category: cs.RO

TL;DR: 论文通过统一框架比较三种基于备份的安全过滤器方法，揭示它们之间的理论联系和差异


<details>
  <summary>Details</summary>
Motivation: 重新审视三种基于备份的安全过滤器方法（Backup CBF、MPS和gatekeeper），通过统一比较框架明确它们的共同备份策略结构和关键算法差异，澄清这些方法之间的理论联系和区别

Method: 使用共同的安全过滤器抽象和共享符号，建立统一比较框架；通过分析过滤器非活动集（即名义策略保持不变的状态集）来比较三种方法；证明MPS是gatekeeper的特例，并将gatekeeper与Backup CBF非活动集内部联系起来

Result: 揭示了基于备份的安全过滤器的一个关键保守性来源：安全性通常通过备份操作的可行性来评估，而不是通过名义策略的持续安全执行来评估；建立了三种方法之间的理论联系，包括MPS是gatekeeper的特例

Conclusion: 论文提供了一个紧凑的教程和综述，澄清了三种基于备份的安全过滤器方法之间的理论联系和差异，强调了它们共同的保守性特征，为理解和比较这些方法提供了统一框架
...
### [25] [A Rapid Instrument Exchange System for Humanoid Robots in Minimally Invasive Surgery](https://arxiv.org/abs/2604.02707)
...
Result: 系统表现出高操作鲁棒性和快速收敛的学习曲线，新手经过简短训练后器械装卸性能显著提升；验证了人形机器人在受限临床环境中稳定执行器械交换的技术可行性

Conclusion: 虽然长距离空间对齐在时间成本和协作稳定性方面仍存在挑战，但该系统成功验证了人形机器人在受限临床环境中执行稳定器械交换的技术可行性
...
### [26] [F2F-AP: Flow-to-Future Asynchronous Policy for Real-time Dynamic Manipulation](https://arxiv.org/abs/2604.02408)
*Haoyu Wei,Xiuwei Xu,Ziyang Cheng,Hang Yin,Angyuan Ma,Bingyao Yu,Jie Zhou,Jiwen Lu*

Main category: cs.RO

TL;DR: 提出基于预测物体流的异步推理框架，通过合成未来观测来补偿延迟，提升动态操作任务的响应性和成功率
...
Method: 提出新颖框架，利用预测物体流合成未来观测，结合基于流的对比学习目标，将预测观测的视觉特征表示与真实未来状态对齐，使异步策略获得前瞻性规划和运动能力

Result: 实验结果表明，该方法在复杂动态操作任务中显著增强了响应性和成功率

Conclusion: 通过预测物体流合成未来观测的框架能够有效补偿异步推理中的延迟问题，使机器人策略在动态环境中具备主动规划和执行能力
...
### [27] [Tune to Learn: How Controller Gains Shape Robot Policy Learning](https://arxiv.org/abs/2604.02523)
*Antonia Bronars,Younghyo Park,Pulkit Agrawal*

Main category: cs.RO

TL;DR: 研究探讨位置控制器增益选择对机器人学习的影响，发现最佳增益取决于学习范式而非任务行为


<details>
  <summary>Details</summary>
Motivation: 传统基于任务刚度选择控制器增益的方法在与状态条件策略结合时失效，因为有效刚度源于学习反应与控制动力学的交互。需要研究增益选择如何影响学习算法的可学习性

Method: 系统研究位置控制器增益对现代机器人学习三大核心组件的影响：行为克隆、从零开始的强化学习、仿真到现实的迁移。在多个任务和机器人平台上进行广泛实验

Result: 1) 行为克隆受益于柔顺和过阻尼增益机制；2) 强化学习在所有增益机制中都能成功，但需要兼容的超参数调优；3) 仿真到现实迁移受到刚性和过阻尼增益机制的损害

Conclusion: 最优增益选择不取决于期望的任务行为，而取决于采用的学习范式。增益选择应基于可学习性而非传统刚度考虑
...
### [28] [From Impact to Insight: Dynamics-Aware Proprioceptive Terrain Sensing on Granular Media](https://arxiv.org/abs/2604.02563)
*Yifeng Zhang,Yue Wu,Jake Futterman,Jacob Meseha,Eduardo Rosales,Irie Cooper,J. Diego Caporale,Feifei Qian*

Main category: cs.RO

TL;DR: 该研究开发了一个基于物理的框架，用于在高速跳跃过程中仅使用本体感知进行动态地形表征，揭示了加速度相关的附加质量效应在高速运动中对颗粒地形力响应的重要影响。


<details>
  <summary>Details</summary>
Motivation: 大多数地形表征方法依赖于准静态假设，忽略了高速运动中的速度和加速度相关效应。机器人需要在高度动态条件下解释接触力，特别是在自然地形中移动时，需要准确理解冲击和快速姿态转换期间产生的力。

Method: 通过控制跳跃实验，系统性地改变冲击速度和腿部刚度，测量颗粒地形相互作用。开发了一个物理框架，将力分解与基于动量观测器的估计器相结合，补偿刚体惯性和重力，并引入加速度感知加权回归来处理高加速度事件期间增加的力方差。

Result: 实验表明，基于准静态的假设在高速跳跃期间会导致颗粒地形属性估计出现较大差异。速度相关的阻力无法解释这些差异，而是加速度相关的附加质量效应（与脚部下方颗粒夹带相关）主导了瞬态力响应。该方法能够在不同运动条件下一致地恢复颗粒刚度参数，与线性执行器地面真实值密切匹配。

Conclusion: 高速运动期间的准确地形推断需要明确处理加速度相关的颗粒效应。该研究为机器人在动态探索陆地和水星环境期间表征复杂可变形地形提供了基础。
...
### [29] [Differentiable SpaTiaL: Symbolic Learning and Reasoning with Geometric Temporal Logic for Manipulation Tasks](https://arxiv.org/abs/2604.02643)
*Licheng Luo,Kaier Liang,Cristian-Ioan Vasile,Mingyu Cai*

Main category: cs.RO

TL;DR: 提出Differentiable SpaTiaL，首个端到端可微符号时空逻辑工具箱，通过构建平滑几何基元实现从高级语义规范到底层几何配置的可微映射


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中执行复杂操作需要满足耦合的几何和时间约束。现有可微时序逻辑忽略交互对象-环境关系，而考虑这些关系的空间逻辑方法依赖离散几何引擎，破坏计算图并阻碍精确梯度传播

Method: 提出完全张量化的工具箱，直接在多边形集合上构建平滑、自动微分兼容的几何基元。通过解析推导关键空间谓词（包括有符号距离、相交、包含和方向关系）的可微松弛版本

Result: 实现了两个核心能力：(i) 在严格时空约束下的大规模并行轨迹优化，(ii) 通过反向传播直接从演示中学习空间逻辑参数。实验验证了框架的有效性和可扩展性

Conclusion: Differentiable SpaTiaL是首个端到端可微符号时空逻辑工具箱，通过完全可微的公式化实现了从高级语义规范到底层几何配置的梯度传播，为机器人操作任务提供了新的优化和学习能力
...
### [30] [V2X-QA: A Comprehensive Reasoning Dataset and Benchmark for Multimodal Large Language Models in Autonomous Driving Across Ego, Infrastructure, and Cooperative Views](https://arxiv.org/abs/2604.02710)
*Junwei You,Pei Li,Zhuoyu Jiang,Weizhe Tang,Zilin Huang,Rui Gan,Jiaxi Liu,Yan Zhao,Sikai Chen,Bin Ran*

Main category: cs.RO

TL;DR: V2X-QA是一个用于评估多模态大语言模型在自动驾驶中多视角推理能力的真实世界数据集和基准，涵盖车辆侧、基础设施侧和协同视角，通过专家验证的多选题标注实现细粒度诊断。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶基准主要关注自我中心视角，无法系统评估模型在基础设施中心和协同驾驶条件下的性能，因此需要开发能够评估多视角推理能力的基准。

Method: 构建V2X-QA数据集，采用视图解耦评估协议，在统一的MCQA框架下支持车辆侧、基础设施侧和协同驾驶条件的受控比较。数据集按12任务分类法组织，涵盖感知、预测、推理和规划。同时提出V2X-MoE基准对齐基线，采用显式视图路由和视角特定的LoRA专家。

Result: 对10个代表性SOTA模型评估显示：视角可访问性显著影响性能；基础设施侧推理支持有意义的宏观交通理解；协同推理仍然具有挑战性，需要跨视图对齐和证据整合而非简单增加视觉输入。V2X-MoE表现出色，表明显式视角专业化是多视图推理的有前景方向。
...
### [31] [Learning Locomotion on Complex Terrain for Quadrupedal Robots with Foot Position Maps and Stability Rewards](https://arxiv.org/abs/2604.02744)
...
Result: 在训练期间见过的地形和域外地形上广泛验证，结果表明所提方法能够实现精确稳定的运动，在域内和域外地形上都提高了运动成功率

Conclusion: 通过结合足部位置映射和动态稳定性奖励的注意力框架，成功解决了四足机器人在复杂地形上运动的精度和稳定性问题
...
TL;DR: 提出一种几何约束的雷达-惯性里程计与建图方法，通过联合整合点和位姿不确定性，利用连续轨迹模型估计任意时间戳的位姿不确定性，并结合异方差测量不确定性进行动态观测置信度评估，自适应降权无信息雷达点，构建高保真地图提升里程计精度。


<details>
  <summary>Details</summary>
Motivation: 雷达里程计在恶劣环境下对鲁棒定位至关重要，但可靠回波的稀疏性和独特的噪声特性限制了其性能。现有方法在处理雷达点云稀疏性和噪声方面存在不足，需要更有效的不确定性建模和几何约束整合方法。

Method: 采用连续轨迹模型估计任意时间戳的位姿不确定性，通过控制点传播不确定性；将位姿不确定性与异方差测量不确定性在点投影过程中连续整合，实现动态观测置信度评估；利用量化不确定性进行雷达建图，构建高保真地图；在不确定性感知建图框架中融入显式几何约束。

Result: 在多样化真实世界数据集上的广泛实验表明，该方法在准确性和效率方面均优于现有基线，实现了显著的性能提升。

Conclusion: 提出的几何约束雷达-惯性里程计与建图方法通过联合不确定性建模和自适应点云处理，有效解决了雷达稀疏性和噪声问题，在不确定性感知建图框架中融入几何约束进一步提升了系统性能。
...
### [33] [Vision-Based End-to-End Learning for UAV Traversal of Irregular Gaps via Differentiable Simulation](https://arxiv.org/abs/2604.02779)
*Linzuo Zhang,Yu Hu,Feng Yu,Yang Deng,Wenxian Yu,Danping Zou*

Main category: cs.RO

TL;DR: 提出了一种完全基于视觉的端到端框架，使无人机能够直接根据深度图像生成控制指令，穿越复杂且未见过的间隙环境。


<details>
  <summary>Details</summary>
Motivation: 无人机在检查、搜救和灾难响应等应用中需要穿越狭窄不规则间隙，但传统方法依赖显式间隙提取和测量，而现有端到端方法通常假设规则形状间隙，导致泛化能力差和实用性有限。

Method: 开发了完全基于视觉的端到端框架，将深度图像直接映射到控制指令；在SE(3)群中操作，结合可微分模拟、停止梯度算子和双峰初始化分布实现稳定穿越；使用两个辅助预测模块（间隙穿越成功分类器和可穿越性预测器）增强连续导航和安全性。

Result: 广泛的模拟和真实世界实验证明了该方法的有效性、泛化能力和实际鲁棒性，能够成功穿越复杂且未见过的间隙环境。

Conclusion: 该框架为无人机在复杂环境中的自主导航提供了实用的解决方案，克服了传统方法和现有端到端方法的局限性，实现了稳定、泛化的间隙穿越能力。
...
### [34] [Learning Structured Robot Policies from Vision-Language Models via Synthetic Neuro-Symbolic Supervision](https://arxiv.org/abs/2604.02812)
*Alessandro Adami,Tommaso Tubaldo,Marco Todescato,Ruggero Carli,Pietro Falco*

Main category: cs.RO

TL;DR: VLM生成可执行行为树策略的神经符号方法，通过合成多模态数据集实现无人工标注监督，在真实机器人上验证了从合成数据到物理系统的成功迁移。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型的端到端视觉运动策略不透明且难以分析，限制了其在安全关键机器人应用中的使用。而经典机器人系统依赖结构化策略表示，提供可解释性、模块化和反应式执行。需要弥合高维学习和符号控制之间的鸿沟。

Method: 提出神经符号方法：VLM从视觉观察、自然语言指令和结构化系统规范合成可执行行为树策略。引入自动化管道生成合成多模态数据集，包含域随机化场景与基础模型生成的指令-策略对，实现无需人工标注的可扩展监督。

Result: 在两个机器人操纵器上的真实世界实验表明，完全从合成监督学习的结构化策略成功迁移到物理系统。基础模型能够生成可解释的结构化机器人策略。

Conclusion: 基础模型可以适应生成可解释的结构化机器人策略，为多模态机器人决策提供不透明端到端方法的替代方案，弥合了高维学习和符号控制之间的鸿沟。
...
### [35] [MFE: A Multimodal Hand Exoskeleton with Interactive Force, Pressure and Thermo-haptic Feedback](https://arxiv.org/abs/2604.02820)
*Ziyuan Tang,Yitian Guo,Chenxi Xiao*

Main category: cs.RO

TL;DR: 本文提出了一种多模态反馈外骨骼（MFE），用于在机器人遥操作中提供力、压力和热反馈，增强用户的情境感知能力。


<details>
  <summary>Details</summary>
Motivation: 当前触觉设备通常只能提供单一模态的反馈，无法同时传递力、压力和热等多种感官信息，限制了机器人遥操作系统的情境感知和用户体验。

Method: 开发了多模态反馈外骨骼（MFE），包含：20个自由度的手部姿态捕捉；主动力反馈机制（产生3.5-8.1N的推拉力）；基于电渗原理的平面执行器（提供压力和振动刺激，最大接触压力2.47kPa）；热电热泵（温度范围10-55°C）。系统集成到X-Arm 6和Inspire Hand机械臂的遥操作系统中。

Result: 用户研究显示，参与者能够成功识别和操作可变形物体，并能区分具有不同温度的远程物体。MFE增强了情境感知能力，提高了机器人遥操作系统的可用性和透明度。

Conclusion: MFE通过提供丰富的多模态触觉反馈（力、压力、热），有效解决了现有触觉设备反馈模式单一的问题，显著提升了机器人遥操作系统的性能和用户体验。
...
### [36] [Orientation Matters: Learning Radiation Patterns of Multi-Rotor UAVs In-Flight to Enhance Communication Availability Modeling](https://arxiv.org/abs/2604.02827)
*Martin Zoula,Daniel Bonilla Licea,Jan Faigl,Václav Navrátil,Martin Saska*

Main category: cs.RO

TL;DR: 提出了一种通过校准飞行数据学习异构四旋翼无人机对天线辐射模式的方法，使用球谐函数或加权平均建模，通过多项式系数线性回归同时解耦两个独立无人机的辐射模式，在真实数据集上达到3.6 dB RMS误差。


<details>
  <summary>Details</summary>
Motivation: 在真实世界应用中，无人机设置经常变化（如有效载荷改变），需要快速重新校准天线辐射模式，以实现精确的自主路径规划和集群控制。现有方法难以同时解耦和学习两个异构无人机的辐射模式。

Method: 1. 使用球谐函数级数或诱导样本加权平均建模天线辐射模式；2. 通过多项式系数线性回归同时解耦两个独立无人机的辐射模式；3. 设计联合校准轨迹，在无障碍无回波高度利用可用飞行时间收集数据。

Result: 在真实世界数据集上评估，成功学习两个辐射模式，达到3.6 dB RMS误差（测量噪声水平），验证了方法的可行性。

Conclusion: 提出的辐射模式学习和解耦方法可用于快速重新校准（如有效载荷变化后），从而在预期设置变化的真实应用中实现精确的自主路径规划和集群控制。
...
### [37] [RAGE: A Tightly Coupled Radar-Aided Grip Estimator For Autonomous Race Cars](https://arxiv.org/abs/2604.02892)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶赛车需要在物理极限下安全有效运行，实时估计车辆-轮胎-路面摩擦至关重要。传统方法依赖昂贵、需要定制安装的专用传感器，限制了可扩展性和部署。

Method: 提出RAGE（实时估计器），仅使用标准传感器（如IMU和雷达）同时推断车辆速度、轮胎侧偏角和作用于轮胎的侧向力。该方法不依赖昂贵专用传感器。

Result: 通过高保真仿真和在EAV-24自动驾驶赛车上进行的真实世界实验验证了该方法，证明了在估计车辆侧向动力学方面的准确性和有效性。
...
### [38] [Learning Task-Invariant Properties via Dreamer: Enabling Efficient Policy Transfer for Quadruped Robots](https://arxiv.org/abs/2604.02911)
*Junyang Liang,Yuxuan Liu,Yabin Chang,Junfan Lin,Junkai Ji,Hui Li,Changxin Huang,Jianqiang Li*

Main category: cs.RO

TL;DR: DreamTIP框架在Dreamer世界模型中引入任务不变属性学习，通过LLM识别接触稳定性、地形间隙等鲁棒属性作为辅助预测目标，结合混合回放缓冲和正则化约束，显著提升四足机器人在复杂地形上的sim-to-real迁移性能。


<details>
  <summary>Details</summary>
Motivation: 解决四足机器人在多样动态地形上运动时，仿真环境与真实世界条件差异带来的挑战。传统sim-to-real方法依赖手动特征设计或昂贵的真实世界微调，需要更高效鲁棒的迁移方法。

Method: 提出DreamTIP框架：1) 利用大语言模型识别任务不变属性（如接触稳定性、地形间隙）；2) 将这些属性作为辅助预测目标集成到Dreamer世界模型中；3) 设计混合回放缓冲和正则化约束的高效适应策略，快速校准真实世界动态，防止表示崩溃和灾难性遗忘。

Result: 在Stair、Climb、Tilt、Crawl等复杂地形上，DreamTIP显著优于现有方法：在8个模拟迁移任务中平均性能提升28.1%；真实世界Climb任务中，基线方法成功率仅10%，而DreamTIP达到100%成功率。

Conclusion: 将任务不变属性整合到Dreamer学习中，为获得鲁棒且可迁移的机器人运动控制提供了新颖解决方案，有效解决了sim-to-real迁移中的动态变化挑战。
...
### [39] [Open-Loop Planning, Closed-Loop Verification: Speculative Verification for VLA](https://arxiv.org/abs/2604.02965)
...
Conclusion: SV-VLA框架有效解决了VLA模型在效率与鲁棒性之间的权衡问题，为动态环境中的具身控制提供了既高效又可靠的解决方案。
...
### [40] [A Flow Matching Framework for Soft-Robot Inverse Dynamics](https://arxiv.org/abs/2604.03006)
...
Method: 提出开环前馈控制的逆动力学框架，将逆动力学重构为条件流匹配问题，采用Rectified Flow作为轻量级实例生成物理一致的控制输入而非条件平均值。引入两个变体增强物理一致性：RF-Physical使用物理先验进行残差建模；RF-FWD在流匹配中集成前向动力学一致性损失。

Result: 框架将轨迹跟踪RMSE降低50%以上（相比MLP、LSTM、Transformer等基线）。系统在1.14 m/s的峰值末端执行器速度下保持稳定开环执行，推理延迟低于1毫秒（0.995 ms）。

Conclusion: 流匹配为软体机器人系统学习微分逆动力学提供了鲁棒、高性能的范式，能够生成物理一致的控制输入，显著提升轨迹跟踪精度和系统性能。
...
### [41] [Asymptotically-Bounded 3D Frontier Exploration enhanced with Bayesian Information Gain](https://arxiv.org/abs/2604.03008)
*John Lewis,Meysam Basiri,Pedro U. Lima*

Main category: cs.RO

TL;DR: 提出了一种基于OctoMap的边界探索算法，具有可预测的渐近有界性能，计算复杂度为O(|F|)，通过贝叶斯回归器估计信息增益，相比现有方法显著提升计算效率


<details>
  <summary>Details</summary>
Motivation: 大规模环境中的机器人探索计算需求高，传统方法复杂度随环境规模增长，需要一种具有可预测性能边界的高效探索算法

Method: 采用OctoMap表示环境，通过战略性的前向和逆向传感器建模实现近似但高效的边界检测和维护，集成贝叶斯回归器估计信息增益以避免显式计数未知体素

Result: 仿真显示该方法比现有OctoMap方法计算效率更高，与独立于OctoMap的基线方法相当，贝叶斯增强框架相比确定性边界基线在不同空间尺度上总探索时间提升达54%

Conclusion: 提出的算法具有可预测的渐近有界性能，计算复杂度与边界数量线性相关，贝叶斯增强显著提升探索效率，实际实验验证了计算边界和增强效果
...
### [42] [Behavior-Constrained Reinforcement Learning with Receding-Horizon Credit Assignment for High-Performance Control](https://arxiv.org/abs/2604.03023)
*Siwei Ju,Jan Tauberschmidt,Oleg Arenz,Peter van Vliet,Jan Peters*

Main category: cs.RO

TL;DR: 提出行为约束强化学习框架，在赛车仿真中学习高性能控制策略，保持与专家行为一致的同时超越演示数据性能


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：强化学习能发现高性能策略但偏离专家行为，模仿学习受限于演示质量且难以超越专家数据。需要一种既能超越演示性能又能控制与专家行为偏差的方法

Method: 提出行为约束强化学习框架，引入滚动时域预测机制建模短期未来轨迹，提供前瞻奖励；通过参考轨迹条件化策略，表示专家一致行为的分布而非单一确定性目标

Result: 在高保真赛车仿真中使用专业车手数据进行评估，学习策略在保持与专家驾驶行为紧密对齐的同时获得有竞争力的圈速，在性能和模仿质量上均优于基线方法；在驾驶员在环仿真中的人类评估显示策略能再现与顶级职业赛车手反馈一致的设置依赖性驾驶特性

Conclusion: 该方法能够学习既最优又行为一致的高性能控制策略，可作为复杂控制系统中人类决策的可靠替代，实现超越演示性能的同时保持专家行为一致性
...
### [43] [ARM: Advantage Reward Modeling for Long-Horizon Manipulation](https://arxiv.org/abs/2604.03037)
...
Method: 提出优势奖励建模（ARM）框架，从难以量化的绝对进度转向估计相对优势；引入三状态标注策略（进步、退步、停滞）减少人工认知负担；训练模型实现自动化进度标注；将ARM集成到离线RL管道中进行自适应动作-奖励重加权

Result: 在挑战性长时程毛巾折叠任务中达到99.4%成功率，相比当前VLA基线在稳定性和数据效率方面均有提升，策略训练期间几乎无需人工干预

Conclusion: ARM框架通过相对优势估计和三状态标注策略有效解决了长时程机器人操作中的奖励稀疏问题，实现了高效、稳定的策略学习
...
### [44] [Enhancing Multi-Robot Exploration Using Probabilistic Frontier Prioritization with Dirichlet Process Gaussian Mixtures](https://arxiv.org/abs/2604.03042)
*John Lewis Devassy,Meysam Basiri,Mário A. T. Figueiredo,Pedro U. Lima*

Main category: cs.RO

TL;DR: 本文提出了一种基于概率前沿优先级的增强方法，通过Dirichlet过程高斯混合模型和信息增益的概率公式，改进了多智能体自主探索中的前沿优先级排序，在多种环境下平均提升10%-14%性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体自主探索在环境监测、搜救等应用中至关重要，但在通信约束下的有效协调仍然是一个重大挑战。现有的前沿探索算法虽然通过分析已知与未知区域的边界来确定最大化探索收益的最佳视角，但在前沿优先级排序方面仍有改进空间。

Method: 提出了一种概率前沿优先级增强方法，采用Dirichlet过程高斯混合模型（DP-GMM）和信息增益的概率公式来改进前沿优先级排序。该方法被集成到两种最先进的多智能体探索算法中，通过概率方法评估前沿区域的信息价值。

Result: 在模拟实验中，该方法在两种算法上分别实现了平均10%和14%的性能提升，覆盖了不同杂乱程度、通信约束和团队规模的环境。在真实世界的双无人机系统实验中成功部署，进一步验证了方法的有效性。

Conclusion: 提出的概率前沿优先级增强方法显著提高了多智能体探索算法的性能，特别是在通信受限的环境中。该方法通过更精确地评估前沿区域的信息价值，实现了更好的探索协调和效率。
...
### [45] [Joint Prediction of Human Motions and Actions in Human-Robot Collaboration](https://arxiv.org/abs/2604.03065)
...
Method: 提出MA-HERP框架：1）层次表示，通过Allen区间关系将运动组合为动作；2）统一概率因子分解，耦合连续动态、离散标签和持续时间；3）受贝叶斯滤波启发的递归推理方案，交替进行自上而下的动作预测和自下而上的感官证据整合。

Result: 基于肌肉骨骼模拟的神经模型初步实验评估显示：准确的运动预测、噪声下的鲁棒动作推断，以及兼容在线人机协作的计算性能。

Conclusion: MA-HERP框架能够有效联合估计和预测人类运动和动作，为人机协作中的实时意图理解提供了可行的解决方案。
...
### [46] [Flash-Mono: Feed-Forward Accelerated Gaussian Splatting Monocular SLAM](https://arxiv.org/abs/2604.03092)
*Zicheng Zhang,Ke Wu,Xiangting Meng,Keyu Liu,Jieru Zhao,Wenchao Ding*

Main category: cs.RO

TL;DR: Flash-Mono是一种基于前馈预测的单目3D高斯溅射SLAM系统，通过多帧上下文直接预测高斯属性，相比传统优化方法实现10倍加速，同时通过2D高斯面元和隐藏状态闭环模块提升几何精度和多视角一致性。


<details>
  <summary>Details</summary>
Motivation: 传统单目3D高斯溅射SLAM存在时间效率低、几何精度差和多视角一致性不足的问题，主要源于每帧从头开始优化的耗时过程以及单帧几何先验缺乏帧间尺度一致性。作者认为需要一种利用多帧上下文直接预测高斯属性的前馈范式来解决这些挑战。

Method: 系统包含三个核心模块：1) 前馈预测前端：训练循环前馈模型，通过交叉注意力逐步聚合多帧视觉特征到隐藏状态，联合预测相机位姿和逐像素高斯属性；2) 2D高斯溅射映射后端：用2D高斯面元替代传统3D高斯椭球体以增强几何保真度；3) 基于隐藏状态的高效闭环模块：利用隐藏状态作为紧凑子图描述符，实现高效闭环和全局Sim(3)优化以缓解漂移问题。

Result: Flash-Mono相比基于优化的GS-SLAM方法实现了10倍加速，同时确保了高质量渲染。通过隐藏状态闭环和全局优化有效缓解了长期漂移问题。在跟踪和建图质量方面均达到了最先进的性能水平。

Conclusion: Flash-Mono通过前馈预测范式成功解决了传统单目3D高斯溅射SLAM的效率、精度和一致性挑战，展示了在具身感知和实时重建应用中的潜力。系统的高效性和高质量表现使其成为该领域的重要进展。
...
### [47] [An Open-Source LiDAR and Monocular Off-Road Autonomous Navigation Stack](https://arxiv.org/abs/2604.03096)
...
TL;DR: 提出一个支持LiDAR和单目3D感知的开源越野导航框架，使用Depth Anything V2进行零样本深度估计，结合VINS-Mono进行度量深度重标定，通过边缘掩码和时间平滑增强鲁棒性，在仿真和真实环境中性能接近高分辨率LiDAR。


<details>
  <summary>Details</summary>
Motivation: 越野自主导航需要可靠的3D感知以在非结构化地形中实现鲁棒的障碍物检测。LiDAR虽然准确但成本高、功耗大，而基于基础模型的单目深度估计提供了轻量级替代方案，但其在户外导航系统中的集成尚未得到充分探索。

Method: 提出开源越野导航框架，支持LiDAR和单目3D感知。单目方案结合零样本深度预测（Depth Anything V2）和稀疏SLAM测量（VINS-Mono）的度量深度重标定。采用边缘掩码减少障碍物幻觉，时间平滑缓解SLAM不稳定性影响。生成点云后创建机器人中心的2.5D高程地图，用于基于代价地图的规划。

Result: 在逼真仿真（Isaac Sim）和真实非结构化环境中评估，单目配置在大多数场景中匹配高分辨率LiDAR性能，表明基于基础模型的单目深度估计是越野导航中可行的LiDAR替代方案。

Conclusion: 通过开源导航框架和仿真环境，提供了完整的越野导航流程和可复现基准。证明基于基础模型的单目深度估计能够替代LiDAR实现鲁棒的越野导航，为低成本、低功耗的越野自主系统提供了可行方案。
...
### [48] [FSUNav: A Cerebrum-Cerebellum Architecture for Fast, Safe, and Universal Zero-Shot Goal-Oriented Navigation](https://arxiv.org/abs/2604.03139)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言导航方法面临异构机器人兼容性差、实时性能不足、导航安全性有限等瓶颈，且难以支持开放词汇语义泛化和多模态任务输入。需要一种能同时解决这些挑战的通用导航框架。

Method: 提出FSUNav大脑-小脑架构：1) 小脑模块：基于深度强化学习的高频端到端通用局部规划器，支持异构平台统一导航；2) 大脑模块：三层推理模型，利用视觉语言模型构建端到端检测验证机制，实现零样本开放词汇目标导航；3) 支持文本、目标描述、图像等多模态输入。

Result: 在MP3D、HM3D和OVON基准测试中，FSUNav在物体导航、实例图像导航和任务导航方面达到最先进性能，显著优于现有方法。在多种机器人平台上的实际部署验证了其鲁棒性和实用性。

Conclusion: FSUNav通过创新的大脑-小脑架构成功解决了异构机器人导航的兼容性、实时性、安全性和开放词汇泛化问题，为通用零样本目标导航提供了有效解决方案，在仿真和真实环境中均表现出优越性能。
...
### [49] [The Compression Gap: Why Discrete Tokenization Limits Vision-Language-Action Model Scaling](https://arxiv.org/abs/2604.03191)
*Takuya Shiba*

Main category: cs.RO

TL;DR: 研究发现，在视觉-语言-动作模型中，当动作表示为离散标记时，升级视觉编码器无法提升下游操作性能，这与视觉-语言建模不同。原因在于"压缩间隙"信息瓶颈原理：离散化动作的固定容量码本成为约束瓶颈，阻止了上游编码器改进的传播。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为，在视觉-语言-动作模型中升级视觉编码器应该像在视觉-语言模型中一样提升下游性能。然而，当动作表示为离散标记时，这种预期可能不成立。研究旨在探索为什么会出现这种差异，并揭示影响物理AI系统扩展性的关键因素。

Method: 提出"压缩间隙"信息瓶颈原理，通过LIBERO基准测试进行三方面验证：1) 因子实验比较Diffusion Policy和OAT在不同模型规模下的性能变化；2) 使用四种不同质量编码器验证编码器质量梯度对两种方法的影响；3) 码本容量实验，通过放松码本容量来检验瓶颈假设的因果证据。

Result: 实验结果显示：Diffusion Policy（连续动作）通过升级视觉编码器性能提升超过21个百分点，而OAT（离散动作）的增益显著衰减；编码器质量梯度实验中，Diffusion Policy性能随编码器质量单调提升，而OAT保持平坦；增加码本容量能部分恢复编码器敏感性，为瓶颈假设提供因果证据。

Conclusion: 物理AI系统的扩展性取决于识别信息流程中的瓶颈位置，而非简单地增加模型或数据规模。当动作离散化时，固定容量的码本成为约束瓶颈，阻止上游视觉编码器改进的传播。这为设计可扩展的视觉-语言-动作系统提供了重要指导。
...
### [50] [Safety-Critical Centralized Nonlinear MPC for Cooperative Payload Transportation by Two Quadrupedal Robots](https://arxiv.org/abs/2604.03200)
...
<details>
  <summary>Details</summary>
Motivation: 解决多机器人协同载荷运输中的安全控制问题，特别是在复杂环境中需要同时确保机器人、载荷和环境障碍物之间的碰撞避免，同时处理系统动力学耦合、质量惯性不确定性和外部扰动。

Method: 1) 将互联机器人-载荷系统建模为离散时间非线性微分代数系统，通过完整约束和交互力矩捕捉耦合动力学；2) 开发基于控制屏障函数的NMPC框架，强制执行机器人和载荷的碰撞避免约束；3) 将交互力矩保留为决策变量，形成结构化DAE约束最优控制问题，实现高效实时计算。

Result: 在Unitree Go2平台上进行了广泛的硬件实验，验证了算法在杂乱环境中协同载荷运输的有效性，能够处理质量和惯性不确定性以及外部推力扰动。

Conclusion: 提出的安全关键集中式NMPC框架成功实现了双四足机器人的协同载荷运输，通过CBF确保安全约束，DAE建模准确捕捉系统耦合，硬件实验证明了其在现实条件下的鲁棒性和实用性。
...
### [51] [KAIJU: An Executive Kernel for Intent-Gated Execution of LLM Agents](https://arxiv.org/abs/2604.02375)
*Cormac Guerin,Frank Guerin*

Main category: cs.SE

TL;DR: KAIJU提出了一种系统级抽象，将LLM智能体工作流执行与推理层解耦，通过意图门控执行和执行内核解决传统ReAct智能体的串行延迟、二次上下文增长和提示注入漏洞问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于ReAct的LLM工具调用智能体存在三个主要限制：串行延迟导致执行效率低下、二次上下文增长消耗大量计算资源、以及易受提示注入和幻觉攻击。现有工作虽然尝试分离规划与执行，但模型仍与执行机制耦合，缺乏系统级的安全保证。

Method: KAIJU引入了两个核心抽象：1) 意图门控执行(IGX)，基于范围、意图、影响和外部批准四个独立变量进行工具授权；2) 执行内核，负责调度、工具分发、依赖解析、故障处理和安全管理。系统采用乐观并行调度，支持三种自适应执行模式（Reflect、nReflect和Orchestrator）。

Result: 与ReAct基线相比，KAIJU在简单查询上因规划开销有延迟惩罚，在中等复杂度任务上性能收敛，在需要并行数据收集的计算查询上具有结构优势。更重要的是，这种分离提供了ReAct仅通过提示无法实现的行为保证。

Conclusion: KAIJU通过系统级抽象成功解耦了LLM智能体的规划与执行，不仅提升了复杂任务的执行效率，更重要的是通过意图门控执行提供了更强的安全保障和行为保证，为LLM智能体系统设计提供了新的架构范式。
...
### [52] [What Are Adversaries Doing? Automating Tactics, Techniques, and Procedures Extraction: A Systematic Review](https://arxiv.org/abs/2604.02377)
*Mahzabin Tamanna,Shaswata Mitra,Md Erfan,Ahmed Ryan,Sudip Mittal,Laurie Williams,Md Rayhanur Rahman*

Main category: cs.SE

TL;DR: 对80篇相关文献的系统性分析，总结了从非结构化文本中提取攻击战术、技术和程序(TTP)的研究现状、趋势和局限性


<details>
  <summary>Details</summary>
Motivation: 攻击者不断演进其战术、技术和程序(TTP)以达成目标并规避检测，防御者需要持续更新对攻击行为的理解。现有研究在提取目标、数据集、建模方法和评估实践上差异很大，难以理解研究现状

Method: 系统分析了80篇同行评审研究，从六个关键维度进行考察：提取目的、数据源、数据集构建、建模方法、评估指标和成果可用性

Result: 发现技术级分类仍是主要任务形式，战术分类和技术搜索研究不足；领域从基于规则和传统机器学习发展到基于Transformer架构，最近研究探索LLM方法；但存在单标签分类、有限评估设置、窄数据集等限制，可复现性受专有数据集、有限代码发布和受限语料库影响

Conclusion: 该研究为安全研究人员提供了从非结构化文本中提取TTP的最新研究现状分析，揭示了当前趋势、进展和局限性，有助于指导未来研究方向
...
### [53] [Ambig-IaC: Multi-level Disambiguation for Interactive Cloud Infrastructure-as-Code Synthesis](https://arxiv.org/abs/2604.02382)
*Zhenning Yang,Kaden Gruizenga,Tongyuan Miao,Patrick Tser Jern Kon,Hui Guan,Ang Chen*

Main category: cs.SE

TL;DR: 提出了一种针对基础设施即代码（IaC）配置生成中模糊性问题的训练无关框架，通过生成多样化候选规格、识别结构分歧、排序信息量并生成针对性澄清问题，逐步缩小配置空间。


<details>
  <summary>Details</summary>
Motivation: 现代云基础设施的规模和复杂性使得基础设施即代码（IaC）对部署管理至关重要。虽然大型语言模型越来越多地用于从自然语言生成IaC配置，但用户请求往往不够明确。与传统代码生成不同，IaC配置无法廉价执行或迭代修复，迫使LLM几乎处于一次性生成模式。研究人员观察到IaC中的模糊性具有可处理的组合结构：配置可分解为三个层次轴（资源、拓扑、属性），其中高层决策约束低层决策。

Method: 提出了一种训练无关、分歧驱动的框架：1）生成多样化的候选规格；2）识别这些轴上的结构分歧；3）按信息量对分歧进行排序；4）生成针对性的澄清问题，逐步缩小配置空间。引入了Ambig-IaC基准测试，包含300个经过验证的具有模糊提示的IaC任务，以及基于图编辑距离和嵌入相似性的评估框架。

Result: 该方法优于最强基线，在结构和属性评估上分别实现了+18.4%和+25.4%的相对改进。

Conclusion: 通过利用IaC模糊性的组合结构，提出的分歧驱动框架能够有效处理用户请求中的不明确性，通过生成有针对性的澄清问题来逐步精化配置，在一次性生成约束下显著提高了IaC配置生成的质量。
...
### [54] [A Synthesis Method of Safe Rust Code Based on Pushdown Colored Petri Nets](https://arxiv.org/abs/2604.02399)
*Kaiwen Zhang,Guanjun Liu*

Main category: cs.SE

TL;DR: 提出基于Pushdown Colored Petri Net的Rust代码合成方法，通过建模所有权、借用和生命周期约束，从公共API签名合成有效的调用序列。


<details>
  <summary>Details</summary>
Motivation: 安全Rust通过编译时约束保证内存安全，但自动合成正确且安全的Rust代码具有挑战性，因为生成的代码必须同时满足所有权、借用、生命周期约束以及类型和接口要求。

Method: 提出Pushdown Colored Petri Net模型，令牌颜色编码动态资源状态和生命周期作用域级别，下推栈跟踪生命周期参数的进入和退出，转移仅在类型匹配、接口义务满足且所需资源状态可用时才被启用。

Result: 基于双模拟理论证明了PCPN的启用和触发规则与这三种约束的编译时检查一致，开发了基于PCPN的自动合成工具，实验结果显示所有合成代码都是正确的。

Conclusion: 提出的PCPN方法能够有效建模Rust的编译时约束，为自动合成安全且正确的Rust代码提供了理论基础和实用工具。
...
### [55] [Improving MPI Error Detection and Repair with Large Language Models and Bug References](https://arxiv.org/abs/2604.02398)
*Scott Piersall,Yang Gao,Shenyang Liu,Liqiang Wang*

Main category: cs.SE

TL;DR: 本文提出了一种结合少样本学习、思维链推理和检索增强生成技术的MPI程序错误检测与修复方法，显著提升了大型语言模型在MPI错误检测中的准确率。


<details>
  <summary>Details</summary>
Motivation: MPI作为高性能计算的基础技术，其程序维护因进程间复杂交互和消息传递同步的复杂性而具有挑战性。虽然大型语言模型如ChatGPT可用于自动化错误检测与修复，但直接应用效果不佳，因为这些模型缺乏MPI程序正确与错误用法的相关知识。

Method: 设计了结合少样本学习(FSL)、思维链推理(CoT)和检索增强生成(RAG)技术的错误检测与修复方法，增强大型语言模型对MPI程序错误的识别和修复能力。

Result: 该方法将错误检测准确率从基准方法的44%显著提升至77%，且实验表明该错误引用技术能够很好地泛化到其他大型语言模型。

Conclusion: 通过集成少样本学习、思维链推理和检索增强生成技术，可以有效增强大型语言模型在MPI程序错误检测与修复方面的能力，显著提升检测准确率并具有良好的模型泛化性。
...
### [56] [TypePro: Boosting LLM-Based Type Inference via Inter-Procedural Slicing](https://arxiv.org/abs/2604.02702)
*Teyu Lin,Minghao Fan,Huaxun Huang,Zhirong Shen,Rongxin Wu*

Main category: cs.SE

TL;DR: TypePro：一种利用LLM进行动态语言类型推断的方法，通过过程间代码切片补充上下文信息，并提出候选复杂类型来解决LLM领域知识不足的问题，在Python和TypeScript数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 动态语言（如Python和JavaScript）的类型处理灵活性带来了类型相关错误和编译时类型推断的额外开销。现有方法仅关注与类型推断直接相关的变量依赖作为上下文，导致上下文信息不完整，影响类型推断准确性。

Method: 提出TypePro方法：1）通过过程间代码切片补充上下文信息；2）基于切片中隐含的数据类型结构信息提出候选复杂类型，解决LLM领域知识不足的问题。

Result: 在ManyTypes4Py和ManyTypes4TypeScript数据集上，Top-1精确匹配率分别达到88.9%和86.6%。相比次优方法，Top-1精确匹配分别提高了7.1和10.3个百分点。

Conclusion: TypePro通过补充上下文信息和提出候选复杂类型，有效解决了动态语言类型推断中上下文信息不完整和LLM领域知识不足的问题，在Python和TypeScript类型推断任务中表现出显著的有效性和鲁棒性。
...
### [57] [Runtime Execution Traces Guided Automated Program Repair with Multi-Agent Debate](https://arxiv.org/abs/2604.02647)
*Jiaqing Wu,Tong Wu,Manqing Zhang,Yunwei Dong,Bo Shen*

Main category: cs.SE

TL;DR: TraceRepair：一个基于多智能体的程序修复框架，利用运行时事实作为约束条件，通过捕获执行快照和委员会交叉验证来修复复杂逻辑错误


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的自动程序修复方法主要依赖静态代码和基本测试输出，难以准确捕捉复杂的运行时行为和动态数据依赖。虽然引入执行跟踪等运行时证据可以暴露具体状态转换，但单个LLM孤立解释这些证据容易过拟合到特定假设，产生仅巧合通过测试而非逻辑正确的补丁。因此，需要将运行时证据作为客观约束而非额外输入。

Method: 提出TraceRepair多智能体框架：1）探测智能体捕获关键变量的执行快照，形成客观修复基础；2）专门化智能体委员会交叉验证候选补丁，暴露不一致性并迭代优化。利用运行时事实作为共享约束进行补丁验证。

Result: 在Defects4J基准测试中，TraceRepair正确修复了392个缺陷，显著优于现有基于LLM的方法。扩展实验表明其在新建的近期bug数据集上具有改进的效率和强泛化能力，确认性能提升源于动态推理而非记忆。

Conclusion: TraceRepair通过将运行时证据作为客观约束，采用多智能体协同验证的方法，有效解决了复杂逻辑错误和静默故障的修复问题，显著提升了自动程序修复的准确性和泛化能力。
...
### [58] [GBQA: A Game Benchmark for Evaluating LLMs as Quality Assurance Engineers](https://arxiv.org/abs/2604.02648)
*Shufan Jiang,Chios Chen,Zhiyang Chen*

Main category: cs.SE

TL;DR: GBQA基准测试评估LLMs在游戏开发中自主发现软件bug的能力，包含30个游戏和124个已验证bug，结果显示当前最佳模型仅能发现48.39%的bug


<details>
  <summary>Details</summary>
Motivation: 自主发现软件bug是现代软件开发的重要挑战，相比代码生成，动态运行时环境的复杂性使得大型语言模型在bug发现方面面临更大困难。游戏开发作为代表性领域，需要建立评估LLMs自主检测软件bug能力的基准测试

Method: 提出游戏质量保证基准测试(GBQA)，包含30个游戏和124个人工验证的bug，分为三个难度级别。使用多智能体系统以可扩展方式开发游戏并注入bug，同时引入人类专家确保正确性。提供基线交互式智能体，配备多轮ReAct循环和记忆机制，支持长时程游戏环境探索以检测bug

Result: 在领先LLMs上的广泛实验表明，自主bug发现仍然极具挑战性：表现最佳的模型Claude-4.6-Opus在思考模式下仅识别出48.39%的已验证bug

Conclusion: GBQA为自主软件工程提供了适当的测试平台和评估标准，在该基准上的进一步进展将有助于缩小自主软件工程领域的差距。当前LLMs在自主bug发现方面仍有很大提升空间
...
### [59] [IndustryCode: A Benchmark for Industry Code Generation](https://arxiv.org/abs/2604.02729)
*Puyu Zeng,Zhaoxi Wang,Zhixu Duan,Liang Feng,Shaobo Wang,Cunxiang Wang,Jinghang Wang,Bing Zhao,Hu Wei,Linfeng Zhang*

Main category: cs.SE

TL;DR: IndustryCode是首个跨工业领域和多编程语言的代码生成基准测试，包含579个子问题，覆盖金融、自动化、航空航天、遥感等领域，支持MATLAB、Python、C++、Stata等语言，用于评估LLM在复杂工业场景中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准测试主要局限于单一领域和语言，无法有效评估LLM在真实工业应用中的泛化能力，也不能反映复杂工业场景所需的编码能力。需要创建跨领域、多语言的工业代码生成基准来填补这一空白。

Method: 构建IndustryCode基准测试，包含579个子问题，源自125个主要工业挑战，每个问题都有严格的问题描述和测试用例。覆盖金融、自动化、航空航天、遥感等多个工业领域，支持MATLAB、Python、C++、Stata等多种编程语言。

Result: 在评估中，表现最佳的模型Claude 4.5 Opus在子问题上达到68.1%的总体准确率，在主问题上达到42.5%的准确率。基准测试数据集和自动化评估代码将在论文接受后公开。

Conclusion: IndustryCode填补了工业代码生成基准测试的空白，为评估LLM在复杂工业场景中的泛化能力提供了首个跨领域、多语言的综合性基准，有助于推动工业智能和决策优化的发展。
...
### [60] [Sustainability Analysis of Prompt Strategies for SLM-based Automated Test Generation](https://arxiv.org/abs/2604.02761)
*Pragati Kumari,Novarun Deb*

Main category: cs.SE

TL;DR: 首次系统评估小型语言模型提示工程策略在自动化测试生成中的可持续性影响，发现提示策略对可持续性结果的影响通常超过模型选择，推理密集型策略带来更高覆盖率但环境成本显著增加


<details>
  <summary>Details</summary>
Motivation: 随着基于提示的自动化测试日益普及，其计算和环境可持续性问题变得重要。现有AI测试可持续性研究主要关注大语言模型，而提示工程策略的影响（特别是在小型语言模型背景下）尚未得到充分探索，但提示设计直接影响推理行为、执行成本和资源利用

Method: 在受控实验设置下，分析三种开源小型语言模型的七种提示策略，联合评估执行时间、令牌使用量、能耗、碳排放和覆盖测试质量（通过对生成测试脚本进行覆盖分析）

Result: 提示策略对可持续性结果有显著且独立的影响，通常超过模型选择的影响。推理密集型策略（如思维链和自洽性）实现更高覆盖率，但执行时间、能耗和碳排放显著更高。相比之下，简单策略（如零样本和ReAct）以显著更低的环境成本提供有竞争力的覆盖测试质量，而Least-to-Most和程序思维提供平衡的权衡

Conclusion: 提示工程策略在小型语言模型驱动的自动化测试生成中具有重要的可持续性影响，需要在实际应用中权衡测试质量与环境成本，选择适当的提示策略可以显著改善测试过程的可持续性
...
### [61] [Evaluating the Environmental Impact of using SLMs and Prompt Engineering for Code Generation](https://arxiv.org/abs/2604.02776)
*Md Afif Al Mamun,Sayan Nath,Gias Uddin,Novarun Deb*

Main category: cs.SE

TL;DR: 本文首次系统研究了不同提示工程策略对小型语言模型代码生成准确性和可持续性（能耗、碳排放）的影响，发现可持续性与准确性往往解耦，简单提示策略如思维链能在推理能力和能源效率间提供接近最优平衡。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型向本地部署的开源小型语言模型转变，AI辅助编程得以普及，但AI的环境足迹也随之分散。虽然提示工程策略（如思维链、ReAct）作为外部机制可优化代码生成而无需修改模型参数，但其对能耗和碳排放的影响对开发者基本不可见。需要系统研究不同提示策略在代码生成准确性和可持续性方面的权衡。

Method: 评估六种主流提示策略在11个开源模型（参数规模1B-34B）上的表现，使用HumanEval+和MBPP+基准测试。同时测量Pass@1准确率、能耗（kWh）、碳排放（kgCO2eq）和推理延迟，分析准确性与可持续性指标之间的关系。

Result: 研究发现可持续性与准确性往往解耦，可在不牺牲性能的情况下实现显著的环境优化。思维链作为较简单的提示技术，能在推理能力和能源效率间提供接近最优平衡。而多重采样策略通常带来不成比例的成本增加，仅获得边际收益。电网碳强度是部署时碳排放的主导因素，表明开发者需要考虑区域能源结构。

Conclusion: 本研究为"绿色"提示工程提供了量化基础，使开发者能够在实现高性能代码生成的同时兼顾生态责任。研究揭示了简单提示策略的环境优势，并强调了考虑区域能源特征的重要性，为可持续AI开发提供了实践指导。
...
### [62] [Dependency-Guided Repository-Level C-to-Rust Translation with Reinforcement Alignment](https://arxiv.org/abs/2604.02852)
*Jia Feng,Wenjie Gan,Cuiyun Gao,Chaozheng Wang,Feng Luo,Xin Xia,Ge Li,Kui Liu*

Main category: cs.SE

TL;DR: DepTrans是一个结合模型能力增强与结构化推理的C到Rust迁移框架，通过强化对齐语法训练和依赖引导迭代优化，在85k数据集上实现60.7%编译成功率和43.5%计算准确率。


<details>
  <summary>Details</summary>
Motivation: 自动化C到Rust迁移对提升软件安全性和性能至关重要。传统基于规则的方法难以处理多样化的C语言习惯用法，生成的Rust代码生硬且不符合习惯。虽然大语言模型通过跨语言泛化能力提供了有前景的替代方案，但仍面临三个主要挑战：1) 现有LLM方法无法有效处理跨文件依赖关系；2) 复杂依赖和结构化输入输出使得仓库级别的语法正确性和功能等价性验证困难；3) 缺乏大规模C-Rust并行数据限制了模型性能。

Method: DepTrans框架结合模型能力增强与结构化推理。采用强化对齐语法训练，通过多任务微调和反馈驱动的强化学习提升生成质量。应用依赖引导迭代优化，捕获细粒度跨文件依赖关系并迭代优化生成的Rust代码。构建了包含85k训练样本的数据集和145个仓库级实例的基准测试。

Result: 实验结果显示，DepTrans实现了60.7%的编译成功率和43.5%的计算准确率，分别比最强基线高出22.8和17.3个百分点。成功构建了15个工业C项目中的7个，展示了实际应用潜力。

Conclusion: DepTrans通过创新的强化对齐语法训练和依赖引导迭代优化，有效解决了C到Rust迁移中的跨文件依赖处理和代码质量提升问题，在编译成功率和计算准确性方面显著优于现有方法，并展示了在工业项目中的实际应用价值。
...
### [63] [BugForge: Constructing and Utilizing DBMS Bug Repository to Enhance DBMS Testing](https://arxiv.org/abs/2604.03024)
*Dawei Li,Qifan Liu,Yuxiao Guo,Jie Liang,Zhiyong Wu,Chi Zhang,Jingzhou Fu,Haogang Mao,Zhenyu Guan,Yu Jiang*

Main category: cs.SE

TL;DR: BugForge框架通过构建标准化的DBMS漏洞仓库并生成高质量测试用例来增强数据库管理系统测试


<details>
  <summary>Details</summary>
Motivation: DBMS是复杂的系统，容易产生可能导致系统故障或数据完整性受损的漏洞。建立统一的DBMS漏洞仓库对于系统化组织漏洞相关数据、改进代码和支持自动化测试至关重要。然而，漏洞报告的异构性以及内容不完整或不准确，使得构建统一仓库并将其转换为高质量测试用例具有挑战性。

Method: BugForge框架采用渐进式收集漏洞报告，然后使用语法感知处理和输入自适应原始PoC提取来构建DBMS漏洞仓库。该仓库存储结构化的漏洞相关数据，包括漏洞元数据和包含潜在漏洞触发语义的原始PoC。这些数据通过语义引导的适应进一步细化为高质量测试用例，从而支持增强的DBMS测试方法，包括DBMS模糊测试、回归测试和跨DBMS漏洞发现。

Result: BugForge为PostgreSQL、MySQL、MariaDB和MonetDB实现，总共集成了37,632个漏洞报告，时间跨度长达28年。基于该仓库，BugForge发现了35个先前未知的漏洞，其中22个被开发者确认，证明了构建和利用漏洞仓库进行DBMS测试的价值。

Conclusion: BugForge框架成功构建了标准化的DBMS漏洞仓库，并能够生成高质量测试用例，有效支持多种DBMS测试方法，显著提升了漏洞发现能力，证明了统一漏洞仓库在DBMS测试中的实用价值。
...
### [64] [Combining Static Code Analysis and Large Language Models Improves Correctness and Performance of Algorithm Recognition](https://arxiv.org/abs/2604.03048)
*Denis Neumüller,Sebastian Boll,David Schüler,Matthias Tichy*

Main category: cs.SE

TL;DR: 结合LLMs与静态代码分析可显著提升算法识别性能，减少72.39-97.50%的LLM调用，F1分数提高达12个百分点，且LLM不依赖标识符名称


<details>
  <summary>Details</summary>
Motivation: 开发者花费大量时间理解源代码，自动识别算法有助于程序理解、维护和软件质量提升。需要评估LLMs与静态分析结合对算法识别的影响

Method: 通过多组实验评估LLMs与静态分析结合，使用不同过滤模式。比较单独使用LLMs的性能，研究不同提示策略的影响，并测试系统化标识符混淆对分类性能和运行时间的影响

Result: LLMs与轻量级静态分析结合表现优异：减少72.39-97.50%的LLM调用，显著降低运行时间；F1分数比基线提高达12个百分点；上下文学习（两个示例）在分类性能和运行效率间达到最佳平衡（F1 75-77%）；LLMs不依赖名称信息，即使标识符混淆仍能识别大多数算法实现

Conclusion: LLMs与静态分析结合可在显著减少运行时间的同时提高F1分数，证明了混合方法的有效性。LLMs不完全依赖名称信息，上下文学习是有效的平衡策略
...
### [65] [PAFT: Preservation Aware Fine-Tuning for Minimal-Edit Program Repair](https://arxiv.org/abs/2604.03113)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在程序修复中存在过度编辑问题，即使生成的补丁能通过测试，也经常修改过多不必要的代码，增加了代码审查和维护成本。标准监督微调缺乏明确的标记级保留信号指导模型区分哪些代码应保留、哪些应修改。

Method: PAFT方法包含三个核心组件：1) 通过对齐错误代码和修复代码推导标记级保留信号；2) 结合全序列掩码技术；3) 应用编辑难度课程学习策略。该方法旨在让模型学习在修复过程中保留稳定上下文，将编辑集中在错误区域。

Result: 在Defects4J和HumanEval-Java基准测试中，PAFT相比标准监督微调将pass@1提高了65.6%，同时将平均编辑距离降低了32.6%。在Defects4J上使用DeepSeek-Coder-6.7B时，PAFT超越了AdaPatcher基线，将pass@1从5.9%提升到10.1%，中位编辑距离从61.0降至42.0。

Conclusion: PAFT能够在不依赖推理时搜索、重排序或后处理的情况下，生成更小、更局部化的合理补丁，有效解决了LLM程序修复中的过度编辑问题，提高了修复的精确性和实用性。
...
### [66] [AI-Assisted Unit Test Writing and Test-Driven Code Refactoring: A Case Study](https://arxiv.org/abs/2604.03135)
*Ema Smolic,Mario Brcic,Luka Hobor,Mihael Kovac*

Main category: cs.SE

TL;DR: 使用AI编码模型进行自动化单元测试生成和后续安全重构的案例研究，通过测试验证代码变更，显著提高测试覆盖率和重构安全性
...
Method: 采用编码模型进行自动化单元测试生成和后续安全重构的案例研究方法，通过迭代生成测试捕获现有系统行为，然后在开发者监督下进行模型辅助重构，使用通过测试验证代码变更

Result: 在数小时内生成近16,000行可靠单元测试（而非数周），关键模块达到78%分支覆盖率，大规模重构期间显著降低回归风险，展示了效率提升和局限性

Conclusion: 该方法展示了软件工程向经验科学的转变，强调数据收集和约束机制，支持快速、安全的迭代开发，为AI辅助重构提供了实践框架
...
### [67] [Backdoor Attacks on Decentralised Post-Training](https://arxiv.org/abs/2604.02372)
*Oğuzhan Ersoy,Nikolay Blagoev,Jona te Lintelo,Stefanos Koffas,Marina Krček,Stjepan Picek*

Main category: cs.CR

TL;DR: 首次提出针对流水线并行的大语言模型去中心化后训练的后门攻击，攻击者仅控制流水线中间阶段而非整个模型或数据集，成功导致模型对齐失效。


<details>
  <summary>Details</summary>
Motivation: 去中心化后训练采用数据和流水线并行技术，但容易受到恶意参与者的中毒和后门攻击。现有研究主要关注数据并行或联邦学习的攻击防御，而对流水线并行的鲁棒性研究仅限于中毒攻击，缺乏后门攻击的研究。

Method: 设计首个针对流水线并行的后门攻击方案，攻击者仅控制流水线中间阶段（而非整个模型或数据集），通过特定触发词注入后门，使训练后的模型产生对齐失效。

Result: 实验表明，即使攻击者权限有限，仍能成功注入后门并导致模型在后训练期间对齐失效。触发词使对齐百分比从80%降至6%。即使对最终模型应用安全对齐训练，后门攻击在60%的情况下仍能成功。

Conclusion: 首次证明流水线并行对后门攻击具有脆弱性，即使攻击者仅控制流水线中间阶段也能成功破坏模型对齐。现有防御措施（如安全对齐训练）对此类攻击效果有限，凸显了去中心化后训练中流水线并行的安全风险。
...
### [68] [Automated Malware Family Classification using Weighted Hierarchical Ensembles of Large Language Models](https://arxiv.org/abs/2604.02490)
*Samita Bai,Hamed Jelodar,Tochukwu Emmanuel Nwankwo,Parisa Hamedi,Mohammad Meymani,Roozbeh Razavi-Far,Ali A. Ghorbani*

Main category: cs.CR

TL;DR: 提出基于加权层次集成预训练大语言模型的零标签恶意软件家族分类框架，通过聚合多LLM决策级预测实现无需标注数据的分类


<details>
  <summary>Details</summary>
Motivation: 现实环境中恶意软件分类面临混淆、加壳和快速演变的挑战，现有基于标注数据、手工特征、监督训练或动态分析的方法在开放世界场景中可扩展性和有效性受限

Method: 采用加权层次集成预训练LLM框架，聚合多个具有互补推理能力的LLM的决策级预测，使用经验推导的macro-F1分数加权，按层次组织：先解决粗粒度恶意行为，再分配细粒度恶意软件家族

Result: 该方法增强了鲁棒性，减少单个模型的不稳定性，并与分析师风格推理保持一致，为开放世界场景提供更有效的分类方案

Conclusion: 基于加权层次集成预训练LLM的零标签框架为恶意软件家族分类提供了有前景的解决方案，特别是在标注数据稀缺的现实环境中
...
### [69] [Opal: Private Memory for Personal AI](https://arxiv.org/abs/2604.02522)
*Darya Kaviani,Alp Eren Ozdarendeli,Jinhao Zhu,Yu Ding,Raluca Ada Popa*

Main category: cs.CR

TL;DR: Opal是一个用于个人AI的隐私内存系统，通过将数据相关推理与大量个人数据分离到可信执行环境中，使用ORAM隐藏访问模式，结合知识图谱提升检索准确性，实现高效隐私保护。


<details>
  <summary>Details</summary>
Motivation: 个人AI系统需要长期存储用户活动数据（文档、邮件、消息等），可信硬件保护隐私但难以扩展，外部存储会暴露检索访问模式泄露隐私。传统ORAM需要固定访问预算，无法支持智能记忆系统所需的查询相关遍历。

Method: Opal的核心洞察是将所有数据相关推理与大量个人数据分离，限制在可信执行环境（enclave）中。不可信磁盘只看到固定的、不可知的（oblivious）内存访问。enclave内组件使用轻量级知识图谱捕获语义搜索遗漏的个人上下文，并通过在每个ORAM访问上搭载重新索引和容量管理来处理持续的数据摄取。

Result: 在基于随机通信模型的综合合成个人数据管道评估中，Opal相比语义搜索将检索准确率提高了13个百分点，相比安全基线实现了29倍吞吐量提升和15倍基础设施成本降低。Opal正在被一家主要AI提供商考虑部署到数百万用户。

Conclusion: Opal通过将数据相关推理限制在可信执行环境中，结合知识图谱和ORAM技术，为个人AI系统提供了既保护隐私又保持高检索准确性的解决方案，解决了传统隐私保护方法在可扩展性和功能性方面的限制。
...
### [70] [From Theory to Practice: Code Generation Using LLMs for CAPEC and CWE Frameworks](https://arxiv.org/abs/2604.02548)
*Murtuza Shahzad,Joseph Wilson,Ibrahim Al Azher,Hamed Alhoori,Mona Rahimi*

Main category: cs.CR

TL;DR: 研究人员利用GPT-4o、Llama和Claude等大语言模型生成与CAPEC和CWE描述对应的漏洞代码片段数据集，包含Java、Python和JavaScript三种语言的615个代码示例，用于增强漏洞理解和训练自动检测模型。


<details>
  <summary>Details</summary>
Motivation: 现有软件漏洞数据集通常缺乏与特定漏洞描述明确关联的全面、详细代码片段，这限制了高级研究的效用，并阻碍了对安全漏洞的深入理解。需要创建更全面的资源来支持漏洞检测和修复研究。

Method: 利用GPT-4o、Llama和Claude等生成式预训练Transformer模型，开发了一种稳健的方法来生成与CAPEC和CWE文档描述的特定漏洞对应的代码片段。通过大语言模型的能力创建漏洞示例。

Result: 生成了包含615个CAPEC代码片段的数据集，涵盖Java、Python和JavaScript三种编程语言。三个模型生成的代码显示出0.98的余弦相似度，表明结果具有一致性。初步评估显示该数据集具有高准确性，可作为漏洞识别系统的可靠参考。

Conclusion: 该数据集不仅增强了代码中安全漏洞的理解，还为专注于自动漏洞检测和修复的机器学习模型训练提供了宝贵资源。这是该领域最广泛和多样化的资源之一，有助于推动漏洞研究的发展。
...
### [71] [SentinelAgent: Intent-Verified Delegation Chains for Securing Federal Multi-Agent AI Systems](https://arxiv.org/abs/2604.02767)
*KrishnaSaiReddy Patil*

Main category: cs.CR

TL;DR: SentinelAgent是一个用于联邦多智能体AI系统中可验证委托链的正式框架，通过委托链演算定义七个属性，并在运行时通过非LLM委托授权服务强制执行，实现100%真阳性率和0%假阳性率。


<details>
  <summary>Details</summary>
Motivation: 现有框架无法回答在复杂委托场景中（如A委托B，B代表用户X调用工具C）授权链的来源和策略违反点，需要建立可验证的委托链框架来解决联邦多智能体系统中的授权追溯和策略合规问题。

Method: 提出委托链演算（DCC）定义七个属性（六个确定性属性+一个概率性意图保持属性），设计意图保持委托协议（IPDP）通过非LLM委托授权服务在运行时强制执行这些属性，采用三点验证生命周期，并使用TLA+模型检查进行机械验证。

Result: 在DelegationBench v4（516个场景，10个攻击类别，13个联邦领域）上实现100%组合真阳性率和0%假阳性率；在黑盒对抗条件下，委托授权服务阻止30/30次攻击且无假阳性；确定性属性在对抗压力测试中不可破坏；意图验证在复杂改写攻击下降至13%，但通过微调NLI模型可将意图保持真阳性率从1.7%提升至88.3%。

Conclusion: SentinelAgent框架为联邦多智能体系统提供了可验证的委托链机制，即使意图验证被规避，其余六个确定性属性仍能约束对手行为，确保系统在授权追溯、策略合规和攻击防御方面的可靠性。
...
### [72] [The Quantum-Cryptographic Co-evolution](https://arxiv.org/abs/2604.02591)
*Ashish Kundu,Ramana Kompella*

Main category: cs.CR

TL;DR: 提出一个二维坐标系来映射密码学韧性与计算能力的协同演化，分析四个象限以分类从传统经典系统到量子安全架构的过渡，强调"量子鸿沟"是最高系统性风险


<details>
  <summary>Details</summary>
Motivation: 随着量子计算向密码学相关量子计算机(CRQC)的实现发展，全球密码基础设施面临生存威胁，需要解决量子计算对现有密码系统的破坏性影响

Method: 引入二维坐标系系统，x轴表示密码学韧性，y轴表示计算能力，通过分析四个象限来分类和映射从传统经典系统到量子安全架构的过渡过程

Result: 识别出"量子鸿沟"（CRQC到达与量子安全采用之间的差距）代表最高系统性风险，需要立即转向密码敏捷框架

Conclusion: 为应对量子计算威胁，必须采用二维坐标系方法来理解密码学韧性与计算能力的协同演化，并优先解决量子鸿沟问题，推动向密码敏捷框架的快速过渡
...
### [73] [ContractShield: Bridging Semantic-Structural Gaps via Hierarchical Cross-Modal Fusion for Multi-Label Vulnerability Detection in Obfuscated Smart Contracts](https://arxiv.org/abs/2604.02771)
*Minh-Dai Tran-Duong,Nguyen Hai Phong,Nguyen Chi Thanh,Doan Minh Trung,Tram Truong-Huu,Van-Hau Pham,Phan The Duy*

Main category: cs.CR

TL;DR: ContractShield：一种针对智能合约漏洞检测的鲁棒多模态框架，通过三级融合机制有效关联语义、时序和结构特征，对抗混淆攻击，在对抗条件下优于现有方法6-15个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有智能合约漏洞检测方法面临两个主要问题：1）攻击者使用虚假代码注入和控制流操纵等混淆技术逃避检测；2）现有多模态方法通常孤立处理不同特征模态，使用简单的拼接融合策略，忽略了跨模态交互，导致当单一模态被混淆时检测精度急剧下降。

Method: 提出ContractShield框架，包含三个核心组件：1）特征提取：使用带滑动窗口的CodeBERT捕获源代码语义依赖，xLSTM建模操作码序列的时序动态，GATv2识别控制流图中的结构不变量；2）三级融合机制：自注意力识别各特征空间内的漏洞模式，跨模态注意力建立模态间互补信号的连接，自适应加权根据混淆条件下的可靠性动态校准特征贡献。

Result: 实验评估显示ContractShield具有强鲁棒性：在混淆数据上获得89%的汉明分数，仅比非混淆数据下降1-3个百分点；同时检测五种主要漏洞类型，F1分数达到91%，在对抗条件下比最先进方法高出6-15个百分点。

Conclusion: ContractShield通过创新的三级融合机制有效关联多模态特征，显著提升了智能合约漏洞检测在对抗混淆攻击下的鲁棒性和准确性，为解决现有方法在跨模态交互和对抗鲁棒性方面的不足提供了有效方案。
...
### [74] [Open Challenges for Secure and Scalable Wi-Fi Connectivity in Rural Areas](https://arxiv.org/abs/2604.02774)
*Philip Virgil Berrer Astillo,Jayasree Sengupta,Mathy Vanhoef*

Main category: cs.CR

TL;DR: 对农村付费Wi-Fi热点系统的安全分析，揭示了连接劫持和恶意热点等实际攻击，提出了安全改进方案
...
Method: 在菲律宾和印度进行Wi-Fi调查了解实际部署情况，对部署进行安全评估，演示两种实际攻击（劫持用户付费连接和恶意热点），分析漏洞根源，引入威胁模型并提出安全改进方案

Result: 发现菲律宾的Piso-WiFi付费热点在农村地区特别普遍，印度的PM-WANI倡议正在缓慢获得关注；成功演示了两种实际攻击，揭示了新兴农村连接基础设施的安全挑战

Conclusion: 付费Wi-Fi热点系统存在严重安全漏洞，需要针对性的安全改进措施，包括安全的缓存架构，以确保农村连接基础设施的安全性和可扩展性
...
### [75] [Towards Secure Agent Skills: Architecture, Threat Taxonomy, and Security Analysis](https://arxiv.org/abs/2604.02837)
*Zhiyuan Li,Jingzheng Wu,Xiang Ling,Xing Cui,Tianyue Luo*

Main category: cs.CR

TL;DR: 首次对Agent Skills框架进行系统性安全分析，揭示其生命周期各阶段的安全风险，构建威胁分类体系，并提出防御建议


<details>
  <summary>Details</summary>
Motivation: Agent Skills作为新兴的开放标准，已在多个智能体平台快速采用并形成大型社区市场，但其安全属性尚未得到系统研究。本文旨在填补这一空白，对Agent Skills框架进行全面的安全分析。

Method: 1. 定义Agent Skill的完整生命周期（创建、分发、部署、执行四个阶段）并识别各阶段的结构性攻击面；2. 基于生命周期分析构建威胁分类体系，包含7个类别和17个场景，覆盖三个攻击层；3. 通过分析Agent Skills生态系统中的5个已确认安全事件验证分类体系；4. 基于发现讨论各威胁类别的防御方向、识别开放研究挑战并提供可操作建议。

Result: 分析揭示最严重的威胁源于框架本身的结构特性：缺乏数据-指令边界、单一审批的持久信任模型、以及缺乏强制性的市场安全审查。这些根本性问题无法通过渐进式缓解措施单独解决。验证了威胁分类体系的有效性，并提供了具体的防御建议。

Conclusion: Agent Skills框架存在系统性安全风险，需要从根本上重新设计安全架构，而不仅仅是渐进式改进。研究为智能体技能生态系统的安全发展提供了重要指导，并指出了需要解决的关键研究挑战。
...
### [76] [Analyzing Healthcare Interoperability Vulnerabilities: Formal Modeling and Graph-Theoretic Approach](https://arxiv.org/abs/2604.03043)
*Jawad Mohammed,Gahangir Hossain*

Main category: cs.CR

TL;DR: 该论文针对HL7 FHIR医疗互操作性平台缺乏并发控制协议的问题，提出了FHIR资源访问图（FRAG）模型，用于检测三种临床相关的竞态条件类型，相比时间窗口基线方法在完全并发访问下实现了64.5个百分点的F1分数提升。


<details>
  <summary>Details</summary>
Motivation: HL7 FHIR医疗互操作性平台支持对共享患者资源的并发异步访问，但FHIR规范缺乏并发控制协议。现有研究仅关注操作系统内核的竞态条件检测，而FHIR安全研究只针对认证和注入攻击，将并发访问视为顺序处理，存在研究空白。

Method: 提出FHIR资源访问图（FRAG）模型，形式化定义为G = (P,R,E,λ,τ,S)，其中节点是并发进程，带类型边表示资源访问事件，竞态条件表示为可检测的结构属性。定义了三种临床相关竞态条件类：同时写入冲突（SWC）、TOCTOU授权违规（TAV）和级联更新竞态（CUR）。实现为三遍图遍历检测算法，并在1,500个合成的FHIR R4事务日志上进行测试。

Result: 在完全并发访问（C2）条件下，FRAG模型达到90.0%的F1分数，而基于时间窗口的基线方法仅为25.5%，提升了64.5个百分点。

Conclusion: FRAG模型有效解决了FHIR平台中并发访问控制的缺失问题，能够检测临床相关的竞态条件，显著优于传统的时间窗口方法，为医疗互操作性平台的安全性和可靠性提供了重要保障。
...
### [77] [Koopman-Based Nonlinear Identification and Adaptive Control of a Turbofan Engine](https://arxiv.org/abs/2604.01730)
*David Grasev*
...
<details>
  <summary>Details</summary>
Motivation: 研究Koopman算子方法在涡扇发动机多变量控制中的应用，解决传统线性化方法在处理非线性系统时的局限性，实现灵活适用于多种控制目标的统一模型框架。

Method: 1) 建立基于物理的部件级模型生成训练数据；2) 开发元启发式扩展动态模态分解方法，设计成本函数准确捕捉转子转速和发动机压力比动态；3) 构建时变Koopman模型；4) 开发自适应Koopman模型预测控制器（带扰动观测器）和Koopman反馈线性化控制器；5) 在海平面和变化飞行条件下评估两种控制策略。

Result: 1) 提出的辨识方法能准确预测转子转速和发动机压力比；2) Koopman模型可在不同控制框架中灵活重用；3) 两种控制策略在稳态条件下性能相当；4) 自适应Koopman模型预测控制器在变化飞行条件下表现出更强的鲁棒性；5) 发动机压力比控制策略改善了推力响应。

Conclusion: 研究证明了Koopman基控制在涡扇发动机控制中的适用性，自适应Koopman模型预测控制器框架在鲁棒性方面具有优势，为非线性航空发动机控制提供了有效解决方案。
...
### [78] [Generating Counterfactual Patient Timelines from Real-World Data](https://arxiv.org/abs/2604.02337)
*Yu Akagi,Tomohisa Seki,Toru Takiguchi,Hiromasa Ito,Yoshimasa Kawazoe,Kazuhiko Ohe*

Main category: cs.LG

TL;DR: 基于30万患者、4亿条时间序列数据的自回归生成模型，可生成临床可信的反事实轨迹，用于模拟不同临床场景下的患者结局


<details>
  <summary>Details</summary>
Motivation: 反事实模拟在个性化医疗和计算机临床试验中具有重要应用前景，但现有方法存在局限性，需要开发能够生成临床可信反事实轨迹的新方法

Method: 使用自监督方式在30万患者、4亿条患者时间序列数据上训练自回归生成模型，应用于COVID-19住院患者，通过修改年龄、C反应蛋白和肌酐水平来模拟7天结局

Result: 反事实模拟显示：年龄增长、C反应蛋白升高和肌酐升高会增加院内死亡率；C反应蛋白升高增加瑞德西韦处方，肾功能受损则减少处方；模拟结果重现了已知临床模式
...
### [79] [LiME: Lightweight Mixture of Experts for Efficient Multimodal Multi-task Learning](https://arxiv.org/abs/2604.02338)
*Md Kowsher,Haris Mansoor,Nusrat Jahan Prottasha,Ozlem Garibay,Victor Zhu,Zhengping Ji,Chen Chen*

Main category: cs.LG

TL;DR: LiME提出了一种轻量级专家混合方法，通过单一共享PEFT模块和轻量级专家向量调制实现专家专业化，无需为每个专家单独适配器，显著减少可训练参数并适用于任何PEFT方法。


<details>
  <summary>Details</summary>
Motivation: 现有MoE-PEFT方法需要为每个专家配备单独的适配器，导致可训练参数随专家数量线性增长，且仅限于基于适配器的架构，限制了其应用范围和效率。

Method: LiME采用单一共享PEFT模块，通过轻量级专家向量调制其输出实现专家专业化；引入零参数路由，利用现有冻结和适应表示消除每层学习路由参数的需求；包含n-gram窗口路由和基于路由置信度的自适应专家选择（Auto Top-K）。

Result: 在包含47个跨文本、图像和视频任务的多模态多任务基准MMT-47上，LiME实现了竞争性或更优的性能，同时使用比相应MoE-PEFT基线少4倍的可训练参数，训练速度提升高达29%。

Conclusion: LiME通过轻量级调制和零参数路由机制，解决了MoE-PEFT方法参数效率低和架构限制的问题，为多任务适应提供了一种更高效、通用的解决方案。
...
### [80] [SIEVE: Sample-Efficient Parametric Learning from Natural Language](https://arxiv.org/abs/2604.02339)
*Parth Asawa,Alexandros G. Dimakis,Matei Zaharia*

Main category: cs.LG

TL;DR: SIEVE是一种从自然语言上下文中进行样本高效参数学习的方法，仅需三个查询示例即可实现模型适应


<details>
  <summary>Details</summary>
Motivation: 自然语言上下文（如指令、知识或反馈）包含丰富的信号用于适应语言模型。虽然上下文学习通过提示提供适应，但参数学习会持久化到模型权重中并能进一步提高性能，但通常需要大量数据，严重依赖高质量轨迹或自动验证器。需要一种样本高效的参数学习方法。

Method: 提出SIEVE方法，使用新颖的合成数据生成管道SIEVE-GEN。该方法基于上下文可分解的洞察：通过将合成查询与仅适用的上下文（而非全部上下文）配对来生成更高质量的rollout，然后使用上下文蒸馏将上下文内化到模型中。

Result: 在需要上下文的推理设置中评估，包括自定义领域以及RuleArena和One Book机器翻译任务。结果显示，SIEVE仅使用三个查询示例就优于先前的上下文蒸馏方法。

Conclusion: SIEVE展示了如何从自然语言中实现样本高效的参数学习，通过上下文分解和蒸馏技术，仅需极少示例即可有效内化上下文知识。
...
### [81] [Not All Denoising Steps Are Equal: Model Scheduling for Faster Masked Diffusion Language Models](https://arxiv.org/abs/2604.02340)
*Ivan Sedykh,Nikita Sorokin,Valentin Malykh*

Main category: cs.LG

TL;DR: 该论文研究如何通过模型调度策略加速掩码扩散语言模型的采样过程，发现用小型模型替换大型模型在扩散过程的早期和晚期步骤中效果更好，可减少17%的计算量而质量下降有限。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型（MDLMs）在质量上已接近自回归语言模型，但其采样过程计算成本高，需要多次全序列去噪过程，且无法像自回归解码那样受益于KV缓存。因此需要寻找方法来加速MDLM的采样过程。

Method: 提出模型调度策略，在部分去噪步骤中用小型MDLM替换大型模型。通过基于损失和KL散度的步骤重要性分析，以及粗粒度步骤段的穷举搜索，确定扩散轨迹中不同步骤对模型替换的敏感度。

Result: 在OpenWebText数据集上，发现早期和晚期去噪步骤对模型替换的鲁棒性显著高于中间步骤，可实现高达17%的FLOPs减少，而生成困惑度仅有适度下降。分析表明扩散轨迹的中间部分最为敏感。

Conclusion: 简单、架构无关的调度规则可以显著加速MDLM采样，同时在生成困惑度指标上基本保持生成质量。这为扩散语言模型的高效采样提供了实用方法。
...
### [82] [LLM Reasoning with Process Rewards for Outcome-Guided Steps](https://arxiv.org/abs/2604.02341)
*Mohammad Rezaei,Jens Lehmann,Sahar Vahdati*

Main category: cs.LG

TL;DR: PROGRS框架通过结果条件中心化处理过程奖励，将过程奖励作为结果组内的相对偏好而非绝对目标，结合冻结分位数回归PRM和多尺度一致性评估器，在数学推理任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习方法主要优化最终答案正确性，但这对多步推理提供稀疏反馈。过程奖励模型(PRMs)虽能评分中间步骤，但常与最终正确性不完全对齐，可能奖励局部流畅但最终错误的推理，导致奖励黑客问题。

Method: 提出PROGRS框架：1) 结果条件中心化，将错误轨迹的PRM分数在每个提示组内零均值化，消除系统偏差同时保留信息性排名；2) 结合冻结分位数回归PRM和多尺度一致性评估器；3) 将中心化过程奖励融入GRPO，无需辅助目标或额外可训练组件。

Result: 在MATH-500、AMC、AIME、MinervaMath和OlympiadBench等数学推理基准上，PROGRS相比仅使用结果奖励的基线一致提升Pass@1，且用更少rollouts实现更强性能。

Conclusion: 结果条件中心化使过程奖励能安全有效地用于数学推理，通过将过程奖励作为结果组内的相对偏好而非绝对目标，解决了PRM与最终正确性不对齐的问题，避免了奖励黑客现象。
...
### [83] [Haiku to Opus in Just 10 bits: LLMs Unlock Massive Compression Gains](https://arxiv.org/abs/2604.02343)
*Roy Rinberg,Annabelle Michael Carrell,Simon Henniger,Nicholas Carlini,Keri Warr*

Main category: cs.LG

TL;DR: 该论文研究LLM生成文本的压缩，探索无损和有损压缩方案，提出基于问答的交互式压缩协议，显著提升压缩效率。


<details>
  <summary>Details</summary>
Motivation: 研究LLM生成文本的压缩问题，探索压缩与计算之间的权衡，旨在开发更高效的压缩方法以减少传输成本，特别是在资源受限环境下。

Method: 1. 无损压缩：使用领域适应的LoRA适配器改进基于LLM的算术编码；2. 有损压缩：先提示模型生成简洁重写，再应用算术编码；3. 问答压缩(QA)：受"二十个问题"游戏启发，小模型通过向强模型询问是/否问题迭代优化响应。

Result: 1. 无损压缩：领域适应LoRA适配器比基础LLM压缩提升2倍；2. 有损压缩：压缩比约0.03，比原始响应压缩提升2倍；3. QA压缩：在8个基准测试中，10个二进制问题可恢复小模型与大模型之间23%-72%的能力差距，压缩比0.0006-0.004，比先前LLM压缩小100倍以上。

Conclusion: 交互式压缩协议比传输完整响应更高效地传递知识，问答压缩展示了在极低比特率下实现显著能力提升的潜力，为资源受限环境中的高效知识传输提供了新途径。
...
### [84] [DrugPlayGround: Benchmarking Large Language Models and Embeddings for Drug Discovery](https://arxiv.org/abs/2604.02346)
*Tianyu Liu,Sihan Jiang,Fan Zhang,Kunyang Sun,Teresa Head-Gordon,Hongyu Zhao*

Main category: cs.LG

TL;DR: DrugPlayGround：一个评估大语言模型在药物发现中性能的基准框架，涵盖药物特性、协同作用、相互作用及生理反应预测


<details>
  <summary>Details</summary>
Motivation: 大语言模型在药物发现研究中日益重要，但目前缺乏客观评估其相对于传统药物发现平台优势和局限性的方法

Method: 开发DrugPlayGround框架，用于评估和基准测试LLM在生成药物特性、药物协同作用、药物-蛋白质相互作用以及药物分子引入扰动后生理反应的文本描述方面的性能

Result: DrugPlayGround能够与领域专家合作，为LLM预测提供详细解释，测试LLM的化学和生物学推理能力

Conclusion: 该框架旨在推动LLM在药物发现各阶段的更广泛应用，通过系统评估提升其在药物研究中的可靠性和实用性
...
### [85] [FTimeXer: Frequency-aware Time-series Transformer with Exogenous variables for Robust Carbon Footprint Forecasting](https://arxiv.org/abs/2604.02347)
*Qingzhong Li,Yue Hu,Zhou Long,Qingchang Ma,Hui Ma,Jinhai Sa*

Main category: cs.LG

TL;DR: 提出FTimeXer，一种频率感知的时间序列Transformer，用于准确预测电网碳强度，解决现有方法在处理非平稳性、周期性模式和异常外生输入方面的不足。


<details>
  <summary>Details</summary>
Motivation: 电网碳强度预测对产品碳足迹核算和脱碳决策至关重要，但现有方法难以有效处理碳强度的高非平稳性、周期性振荡模式，以及在面对缺失数据或错位等不规则外生输入时表现不佳。

Method: 提出FTimeXer模型，包含FFT驱动的频率分支与门控时频融合机制，有效捕捉多尺度周期性；采用随机外生掩码与一致性正则化的鲁棒训练方案，减少虚假相关性并增强稳定性。
...
### [86] [Contextual Intelligence The Next Leap for Reinforcement Learning](https://arxiv.org/abs/2604.02348)
*André Biedenkapp*

Main category: cs.LG

TL;DR: 论文提出了一种新的上下文分类法，将上下文分为异源（环境施加）和自源（智能体驱动）因素，并提出了实现真正上下文智能的三个研究方向：异构上下文学习、多时间尺度建模和抽象高层上下文集成。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在游戏、机器人和连续控制等领域取得了显著成果，但学习到的策略往往无法泛化到训练分布之外，限制了实际应用。现有的上下文强化学习将上下文视为单一、静态的观测变量，这种方法限制了智能体的泛化能力。需要更精细的上下文建模来提升智能体的上下文智能。

Method: 提出了一种新颖的上下文分类法，将上下文分为异源（环境施加）和自源（智能体驱动）因素。在此基础上，提出了三个关键研究方向：1) 异构上下文学习，利用分类法层次使智能体能够推理上下文对世界的影响及反之；2) 多时间尺度建模，识别异源变量演化缓慢或保持静态，而自源变量可能在单次情节内变化；3) 抽象高层上下文集成，纳入角色、资源与监管机制、不确定性等非物理描述符。

Result: 论文提出了一个系统性的框架，将上下文作为一等建模原语，使智能体能够推理自身身份、世界允许的行为以及两者随时间的演化。这为开发新一代上下文感知智能体奠定了基础，这些智能体能够在现实世界中安全高效地部署。

Conclusion: 通过将上下文视为核心建模原语，并采用提出的分类法和研究方向，可以催化新一代上下文感知智能体的发展。这些智能体能够理解自身在环境中的角色、环境的约束条件以及它们随时间的演化，从而实现更好的泛化能力和现实世界部署。
...
### [87] [A Spectral Framework for Multi-Scale Nonlinear Dimensionality Reduction](https://arxiv.org/abs/2604.02535)
*Zeyang Huang,Angelos Chatzimparmpas,Thomas Höllt,Takanori Fujiwara*

Main category: cs.LG

TL;DR: 提出一种新的非线性降维光谱框架，解决全局-局部保持权衡和表达能力-分析透明度差距问题


<details>
  <summary>Details</summary>
Motivation: 解决降维中的两个长期权衡：1) 全局-局部保持矛盾（t-SNE/UMAP侧重局部但扭曲全局，Laplacian Eigenmaps保持全局但局部分离有限）；2) 表达能力与分析透明度之间的差距（许多非线性降维方法缺乏与高维结构的明确联系）

Method: 提出光谱框架，结合光谱基和交叉熵优化进行高维数据嵌入，支持多尺度表示以桥接全局和局部结构。利用线性光谱分解，通过图频率视角分析嵌入，研究光谱模式如何影响嵌入结果。辅以基于字形（glyph）的散点图增强进行可视化探索

Result: 定量评估和案例研究表明，该框架改善了流形连续性，同时通过光谱模式贡献实现了对嵌入结构的更深入分析

Conclusion: 提出的光谱框架解决了降维中的关键权衡问题，既保持了全局-局部结构的平衡，又提供了更好的分析透明度，为非线性降维提供了新的分析视角
...
### [88] [OPRIDE: Offline Preference-based Reinforcement Learning via In-Dataset Exploration](https://arxiv.org/abs/2604.02349)
...
<details>
  <summary>Details</summary>
Motivation: 偏好强化学习(PbRL)能避免复杂的奖励设计并更好对齐人类意图，但获取人类偏好反馈成本高、耗时长，成为PbRL应用的主要障碍。离线PbRL中查询效率低主要源于探索效率低下和奖励函数过优化两个问题。

Method: 提出OPRIDE算法，包含两个关键特征：1) 最大化查询信息量的原则性探索策略；2) 缓解奖励函数过优化的折扣调度机制。算法在离线设置下运行，通过理论保证效率。

Result: 实验表明OPRIDE显著优于现有方法，在更少查询下实现强大性能。在多种运动、操作和导航任务中验证了方法的有效性和通用性。

Conclusion: OPRIDE有效解决了离线PbRL的查询效率问题，通过信息最大化探索和折扣调度机制，显著减少所需人类反馈，为实际应用提供了高效解决方案。
...
### [89] [Modeling and Controlling Deployment Reliability under Temporal Distribution Shift](https://arxiv.org/abs/2604.02351)
*Naimur Rahman,Naazreen Tabassum*

Main category: cs.LG

TL;DR: 提出部署中心框架，将机器学习可靠性视为动态状态（区分度和校准），通过干预策略平衡可靠性稳定性与累积干预成本


<details>
  <summary>Details</summary>
Motivation: 非平稳环境中的机器学习模型面临时间分布漂移，会侵蚀预测可靠性。现有缓解策略（如定期重训练和重新校准）通常关注孤立时间点的平均指标，未明确建模可靠性在部署期间的演变过程

Method: 提出部署中心框架，将可靠性视为由区分度和校准组成的动态状态。通过顺序评估窗口中的状态轨迹定义可测量的波动性，将部署适应问题表述为多目标控制问题，平衡可靠性稳定性与累积干预成本。定义状态依赖的干预策略族，并在大规模时间索引信用风险数据集（135万笔贷款，2007-2018）上进行实验

Result: 选择性、漂移触发的干预策略能够比连续滚动重训练获得更平滑的可靠性轨迹，同时显著降低运营成本。实验展示了成本-波动性帕累托前沿
...
### [90] [An Initial Exploration of Contrastive Prompt Tuning to Generate Energy-Efficient Code](https://arxiv.org/abs/2604.02352)
*Sophie Weidmann,Fernando Castor*

Main category: cs.LG

TL;DR: 该研究提出对比提示调优（CPT）方法，通过结合对比学习和提示调优技术，优化大语言模型生成更节能的代码，以支持绿色软件开发。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成的代码虽然功能正确，但相比人工编写的解决方案能耗更高，这与绿色软件开发（GSD）减少代码能耗的目标相冲突。研究旨在探索如何优化LLM以促进生成节能代码。

Method: 采用对比提示调优（CPT）方法，结合对比学习技术（帮助模型区分高效和低效代码）和提示调优（一种参数高效微调方法，成本仅为传统微调的一小部分）。在Python、Java和C++三种语言的编码问题上对三个不同模型进行了全面评估。

Result: CPT方法在两个模型上实现了代码准确性的持续改进，但效率提升因模型、语言和任务复杂度而异，表明改进并非始终可靠。效率增益在不同情况下表现不一致。

Conclusion: 虽然CPT方法在提升代码准确性方面表现一致，但在生成节能代码方面的效果受多种因素影响，需要进一步研究以实现更可靠的节能代码生成优化。
...
### [91] [From Broad Exploration to Stable Synthesis: Entropy-Guided Optimization for Autoregressive Image Generation](https://arxiv.org/abs/2604.02355)
*Han Song,Yucheng Zhou,Jianbing Shen,Yu Cheng*

Main category: cs.LG

TL;DR: 提出EG-GRPO方法，通过熵分析指导CoT与RL在文本到图像生成中的交互，实现更优的生成质量


<details>
  <summary>Details</summary>
Motivation: 虽然结合思维链和强化学习能改进文本到图像生成，但两者之间的交互机制尚不明确，需要系统性分析来理解探索与优化的动态关系

Method: 进行熵基分析揭示三个关键发现，并基于此提出熵引导的组相对策略优化方法，根据不确定性重新分配优化预算

Result: 在标准文本到图像基准测试中，EG-GRPO实现了最先进的性能表现

Conclusion: 熵分析为理解CoT与RL交互提供了系统框架，EG-GRPO通过熵引导的优化策略有效平衡探索与稳定性，提升了文本到图像生成质量
...
### [92] [Dynamical structure of vanishing gradient and overfitting in multi-layer perceptrons](https://arxiv.org/abs/2604.02393)
*Alex Alì Maleknia,Yuzuru Sato*

Main category: cs.LG

TL;DR: 论文通过最小化模型研究多层感知机中的梯度消失和过拟合问题，揭示了学习动态会经过平台区和近最优区等鞍点结构，最终必然收敛到过拟合解。


<details>
  <summary>Details</summary>
Motivation: 梯度消失和过拟合是机器学习中最广泛研究的问题，但现有研究多在渐近设定下进行，这掩盖了导致这些问题出现的基本动态机制。本文旨在为多层感知机中的学习过程提供清晰的动态描述。

Method: 引入受Fukumizu和Amari研究启发的简化模型，研究通过梯度下降训练的多层感知机中的梯度消失和过拟合。该模型允许分析学习动态，包括平台区域和近最优区域等鞍点结构。

Result: 学习动态在训练过程中会经过平台区域和近最优区域（两者都由鞍点结构组成），最终收敛到过拟合区域。在训练数据集满足适当条件下，过拟合区域以高概率坍缩为单一吸引子（模对称性）。任何在有限噪声数据集上训练的MLP都无法收敛到理论最优解，而必然收敛到过拟合解。

Conclusion: 过拟合不是训练过程中的偶然现象，而是多层感知机在有限噪声数据集上训练的必然结果。该研究为理解梯度消失和过拟合提供了动态视角，揭示了这些现象的根本机制。
...
### [93] [Mitigating Data Scarcity in Spaceflight Applications for Offline Reinforcement Learning Using Physics-Informed Deep Generative Models](https://arxiv.org/abs/2604.02438)
*Alex E. Ballentine,Nachiket U. Bapat,Raghvendra V. Cowlagi*

Main category: cs.LG

TL;DR: 提出MI-VAE方法解决航天控制中sim-to-real差距问题，通过物理信息生成模型增强有限真实数据，提升RL控制器泛化能力


<details>
  <summary>Details</summary>
Motivation: 强化学习控制器在物理系统部署中面临仿真到现实的泛化差距，在航天领域尤为严重，因为真实数据稀缺且成本高昂。传统方法如系统辨识和合成数据生成依赖充足数据且常因建模假设或缺乏物理约束而失败。
...
Conclusion: 这项工作展示了在复杂、数据受限环境中增强自主控制器鲁棒性的可扩展策略，通过物理信息生成模型有效缓解sim-to-real差距。
...
### [94] [Matrix Profile for Time-Series Anomaly Detection: A Reproducible Open-Source Benchmark on TSB-AD](https://arxiv.org/abs/2604.02445)
*Chin-Chia Michael Yeh*

Main category: cs.LG

TL;DR: MMPAD是一个基于矩阵轮廓(Matrix Profile)的时间序列异常检测开源系统，针对TSB-AD基准测试设计，结合了多维聚合、高效的k近邻检索和移动平均后处理技术。


<details>
  <summary>Details</summary>
Motivation: 矩阵轮廓方法虽然具有可解释性和可扩展性，但在时间序列异常检测中的基准性能仍然依赖于超越基本最近邻轮廓的设计选择。该技术报告旨在提供一个可复现的基于矩阵轮廓的异常检测参考系统，并参与TSB-AD基准测试。

Method: 系统结合了三种关键技术：1)预排序的多维聚合；2)高效的排除区感知k近邻检索，用于处理重复异常；3)移动平均后处理。系统针对单变量和多变量时间序列分别配置了超参数设置。

Result: 提供了完整的开源实现，详细记录了在TSB-AD基准测试中的超参数设置和对应结果。分析了系统在总体排行榜上的表现以及在不同数据集特性上的性能。

Conclusion: MMPAD为基于矩阵轮廓的时间序列异常检测提供了一个可复现的参考实现，通过结合多项优化技术提升了基准性能，并公开了完整的实现代码供研究社区使用。
...
### [95] [Do We Need Frontier Models to Verify Mathematical Proofs?](https://arxiv.org/abs/2604.02450)
*Aaditya Naik,Guruprerana Shabadi,Rajeev Alur,Mayur Naik*

Main category: cs.LG

TL;DR: 论文研究了LLM在数学证明验证中的能力，发现开源模型在准确性上仅比前沿模型落后约10%，但在一致性上落后达25%。通过LLM引导的提示搜索，开发了专门提示词集合，将小模型的性能提升9.1%准确性和15.9%一致性。


<details>
  <summary>Details</summary>
Motivation: 随着前沿推理模型在数学竞赛和解决开放问题方面取得突破，需要验证其自然语言证明的正确性。LLM裁判被广泛采用来评估这些证明，但验证所需的具体模型能力尚不明确。研究旨在系统评估不同LLM在验证竞赛级数学证明时的准确性和一致性表现。

Method: 系统评估了4个开源模型和2个前沿LLM在人类评分的自然语言证明数据集上的表现。采用两个关键指标：验证器准确性和自我一致性（同一证明重复判断的一致性率）。通过LLM引导的提示搜索，合成专门提示词集合来克服小模型的特定失败模式。

Result: 发现开源模型在准确性上仅比前沿模型落后约10%，但在自我一致性上落后达25%。所有模型的验证准确性都对提示选择敏感。通过专门提示词集合，小模型的性能可提升9.1%准确性和15.9%一致性，使Qwen3.5-35B等模型达到与Gemini 3.1 Pro相当的水平。

Conclusion: 小模型实际上具备与前沿模型相当的数学验证能力，但难以通过通用判断提示可靠地激发这些能力。专门设计的提示词集合可以显著提升小模型的验证性能，使其在证明验证任务上达到前沿模型的水平，这为高效利用较小模型进行可靠证明验证提供了实用方法。
...
### [96] [On the Geometric Structure of Layer Updates in Deep Language Models](https://arxiv.org/abs/2604.02459)
*Jun-Sik Yoo*

Main category: cs.LG

TL;DR: 研究发现深度语言模型的层间更新可分解为主导的tokenwise成分和几何上不同的残差成分，后者承载了重要的功能计算。


<details>
  <summary>Details</summary>
Motivation: 研究深度语言模型中层间更新的几何结构，而非中间表示编码的信息，旨在理解表示如何从一层变化到下一层。

Method: 提出一个架构无关的框架，将层间更新分解为tokenwise成分和残差成分，分析两者的几何对齐、角度偏差和投影特性，并评估近似误差与输出扰动的关系。

Result: 层间更新几乎完美对齐于tokenwise成分，而残差成分对齐较弱、角度偏差较大、在主导子空间投影较低。近似误差与输出扰动强相关（Spearman相关系数常超过0.7，大模型中可达0.95）。

Conclusion: 大多数层间更新表现为沿主导方向的结构化重参数化，而功能上重要的计算集中在几何上不同的残差成分中，这为理解现代语言模型的层更新结构提供了新视角。
...
TL;DR: 该论文提出了一种基于时间扭曲的RNN迁移学习方法，证明了LSTM可以准确近似时间滞后模型，并在野火建模的燃料水分含量预测应用中验证了该方法，仅修改少量参数即可达到与传统方法相当的精度。


<details>
  <summary>Details</summary>
Motivation: 物理系统的动态演化在不同环境条件下会有不同的时间尺度，传统迁移学习方法需要大量参数调整。研究旨在开发一种基于时间扭曲的迁移学习方法，通过简单的时间尺度调整来适应不同时间特性的系统，减少参数修改量。

Method: 提出基于时间扭曲的RNN迁移学习方法：1) 证明LSTM可以任意精度近似线性一阶微分方程（时间滞后模型）；2) 通过时间扭曲（时间尺度重缩放）调整模型以适应不同时间尺度；3) 在燃料水分含量预测应用中，先在10小时时间尺度的燃料数据上预训练LSTM，然后通过时间扭曲调整模型以预测1小时、100小时和1000小时时间尺度的燃料；4) 与传统迁移学习方法进行对比评估。

Result: 1) 理论证明LSTM可以准确近似时间滞后模型，且时间扭曲后仍能保持近似精度；2) 在燃料水分含量预测应用中，时间扭曲方法仅修改少量参数，但预测精度与传统迁移学习方法相当；3) 该方法在适应不同时间尺度系统时表现出高效性和有效性。

Conclusion: 基于时间扭曲的迁移学习方法为RNN适应不同时间尺度的物理系统提供了一种有效途径，仅需少量参数调整即可达到与传统方法相当的精度，在野火建模等时间尺度敏感的应用中具有实用价值。
...
### [98] [SEDGE: Structural Extrapolated Data Generation](https://arxiv.org/abs/2604.02482)
*Kun Zhang,Jiaqi Sun,Yiqing Li,Ignavier Ng,Namrata Deka,Shaoan Xie*

Main category: cs.LG

TL;DR: 提出基于数据生成过程假设的结构外推数据生成框架SEDGE，确保满足新规范的数据可靠生成及其分布近似可识别性，并提供基于结构优化或扩散后验采样的实用方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据生成方法在外推到新规范时面临挑战，需要建立理论保证和实用方法，确保生成数据满足新规范要求且分布可识别。

Method: 基于数据生成过程的结构假设，提出SEDGE框架：1) 理论层面建立新规范数据可靠生成条件及其分布近似可识别性；2) 算法层面开发基于结构信息优化策略或扩散后验采样的实用外推数据生成方法。

Result: 在合成数据上验证外推性能，并在外推图像生成这一真实场景中展示框架有效性，证明所提方法能够可靠生成满足新规范的数据。

Conclusion: SEDGE框架为结构外推数据生成提供了理论保证和实用算法，能够在保守假设下可靠生成满足新规范的数据并近似识别其分布，在合成和真实场景中均验证了有效性。
...
### [99] [Re-analysis of the Human Transcription Factor Atlas Recovers TF-Specific Signatures from Pooled Single-Cell Screens with Missing Controls](https://arxiv.org/abs/2604.02511)
*Arka Jain,Umesh Sharma*

Main category: cs.LG

TL;DR: 重新分析人类TF Atlas数据集，开发可重复流程进行质量控制、条形码解复用、差异表达和功能富集分析，使用外部对照恢复TF特异性信号，验证数据集可用于可靠的TF功能研究
...
Method: 重新分析GSE216481数据集，开发可重复流程进行质量控制、MORF条形码解复用、TF特异性差异表达和功能富集分析；使用胚胎体细胞作为外部基线，通过背景减除去除批次/转导伪影

Result: 从77,018个细胞中成功分配60,997个（79.2%）到87个TF身份；恢复59/61个可测试TF的特异性信号（vs. 仅27个通过one-vs-rest检测）；识别HOPX、MAZ、PAX6、FOS、FEZF2为最强转录重塑因子；发现TF特异性富集模式；与已发表排名显著一致（Spearman ρ=-0.316, p=0.013）
...
### [100] [AdaHOP: Fast and Accurate Low-Precision Training via Outlier-Pattern-Aware Rotation](https://arxiv.org/abs/2604.02525)
*Seonggon Kim,Alireza Khodamoradi,Kristof Denolf,Eunhyeok Park*

Main category: cs.LG

TL;DR: AdaHOP提出自适应Hadamard变换策略，根据LLM中不同张量的异常值模式选择最优量化方案，在MXFP4精度下达到BF16训练质量，实现3.6倍内存压缩和1.8倍内核加速。


<details>
  <summary>Details</summary>
Motivation: 现有低精度训练方法采用固定的Hadamard变换来抑制异常值和减少量化误差，但忽略了不同张量间异常值结构的显著差异。研究发现异常值模式在权重、激活和梯度中变化很大，固定变换策略存在根本缺陷。

Method: 首先系统研究LLM中权重、激活和梯度的异常值模式，将其分为行向、列向和无异常三种类型。基于此提出AdaHOP：根据每个矩阵乘法的异常值模式分配最优策略：当内维度平滑有效时使用内Hadamard变换；否则结合选择性异常值提取，将主导异常值路由到高精度路径。配合硬件感知的Triton内核实现。

Result: AdaHOP在MXFP4精度下达到BF16训练质量，相比BF16全精度训练实现高达3.6倍内存压缩和1.8倍内核加速。

Conclusion: 异常值模式在LLM的不同张量中存在显著差异，需要针对性的处理策略。AdaHOP通过自适应Hadamard变换和异常值模式感知策略，有效解决了低精度训练中的量化误差问题，在保持训练质量的同时显著提升了效率和内存利用率。
...
### [101] [Jump Start or False Start? A Theoretical and Empirical Evaluation of LLM-initialized Bandits](https://arxiv.org/abs/2604.02527)
*Adam Bayley,Xiaodan Zhu,Raquel Aoki,Yanshuai Cao,Kevin H. Wilson*

Main category: cs.LG

TL;DR: 研究LLM生成偏好数据在注入随机和标签翻转噪声时的表现，发现对齐领域在30%污染内有效，40%失去优势，50%以上性能下降；系统不对齐时LLM先验可能导致比冷启动更高的遗憾


<details>
  <summary>Details</summary>
Motivation: 现有研究假设LLM生成的用户偏好数据与实际偏好合理对齐，但缺乏对噪声和不对齐情况的系统性分析。需要研究LLM生成数据在不同噪声水平下的表现，以及系统不对齐对上下文多臂赌博机性能的影响。

Method: 通过向LLM生成的合成训练数据注入随机和标签翻转噪声，系统性地测试不同噪声水平下的性能。开发理论分析框架，分解随机标签噪声和系统不对齐对先验误差的影响，推导LLM热启动优于冷启动的充分条件。在多个联合数据集和LLM上进行验证。

Result: 在对齐领域，热启动在30%污染内保持有效，40%左右失去优势，超过50%性能下降。当存在系统不对齐时，即使没有额外噪声，LLM生成的先验也可能导致比冷启动更高的遗憾。估计的对齐度能可靠地追踪热启动何时改善或降低推荐质量。

Conclusion: LLM生成偏好数据的热启动效果受噪声和不对齐程度显著影响。理论分析提供了LLM热启动优于冷启动的充分条件，对齐度估计可作为实际应用中判断热启动有效性的可靠指标。
...
TL;DR: 提出一种轻量级共享内存优化技术，解决NF4量化模型在NVIDIA GPU上推理时的性能瓶颈，通过内存层次结构利用实现2-2.2倍内核加速，保持完整生态系统兼容性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型已超出单GPU设备的内存容量，需要量化技术进行实际部署。虽然NF4量化可实现4倍内存减少，但在当前NVIDIA GPU上进行推理时需要昂贵的反量化回FP16格式，这造成了关键性能瓶颈。

Method: 提出轻量级共享内存优化技术，通过原则性的内存层次结构利用来解决这一差距。该技术利用共享内存相对于全局内存12-15倍的延迟优势，通过简化索引逻辑减少指令计数，每个线程块仅使用64字节共享内存。

Result: 在三个模型（Gemma 27B、Qwen3 32B和Llama3.3 70B）上实现了2.0-2.2倍的内核加速，通过利用共享内存优势实现高达1.54倍的端到端改进。与开源BitsAndBytes实现相比，展示了显著性能提升。

Conclusion: 轻量级优化能以最小工程努力提供实质性性能增益。这项工作为HuggingFace生态系统提供了一个即插即用的解决方案，使现有GPU基础设施能够更高效地运行先进模型。
...
TL;DR: ROMAN是一种确定性时间序列算子，通过构建抗混叠多尺度金字塔，将时间尺度和粗粒度时间位置映射到显式通道结构中，同时减少序列长度，为卷积分类器提供紧凑的多尺度表示。


<details>
  <summary>Details</summary>
Motivation: 标准卷积分类器在处理时间序列时通常通过池化操作获得时间不变性，但这会抑制对时间结构（如粗粒度位置、长程相关性、多尺度交互）的敏感性。需要一种机制来显式控制下游模型的归纳偏置，使其能够更好地捕捉时间序列中的结构信息。

Method: ROMAN算子构建抗混叠多尺度金字塔，从每个尺度提取固定长度的窗口，并将它们堆叠为伪通道，形成紧凑表示。该表示将时间尺度和粗粒度时间位置映射到通道维度，同时缩短处理的时间轴。标准卷积分类器可直接在此表示上操作。

Result: 在合成任务中，ROMAN在需要粗粒度位置感知、长程相关性、多尺度交互和完全位置不变性的场景中表现一致，特别适用于标准池化卷积倾向于抑制的时间结构信息。在UCR和UEA档案的长序列子集上，ROMAN的效果因任务而异，但在计算效率方面通常具有优势。

Conclusion: ROMAN提供了一种简单有效的机制来控制卷积分类器的归纳偏置，使其能够更好地处理时间序列中的结构信息。它通过将多尺度表示映射到通道维度，在保持或提升性能的同时提高了计算效率，为时间序列分类提供了一种实用的替代表示方法。
...
<details>
  <summary>Details</summary>
Motivation: 评估3D空间推理的代码生成模型需要在实际环境中执行生成的代码，并评估超越表面正确性的输出质量。现有方法缺乏统一的评估平台

Method: 开发VoxelCode平台，集成自然语言任务规范、Unreal Engine的API驱动代码执行，以及支持自动指标和人工评估的统一评估流水线。构建VoxelCodeBench基准测试，包含符号解释、几何构造和艺术创作三个推理维度的体素操作任务

Result: 评估领先的代码生成模型发现：生成可执行代码远比生成空间正确的输出容易，几何构造和多对象组合任务尤其具有挑战性
...
### [105] [Steerable but Not Decodable: Function Vectors Operate Beyond the Logit Lens](https://arxiv.org/abs/2604.02608)
*Mohammed Suhail B Nadaf*

Main category: cs.LG

TL;DR: 研究发现函数向量（FVs）即使在对数透镜无法解码正确答案时仍能成功引导模型行为，揭示了FVs编码的是计算指令而非答案方向，且不同模型家族在引导机制上存在差异。


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证假设：函数向量引导失败反映了任务相关信息的缺失，即对数透镜会与引导同时失败。通过大规模跨模板FV转移研究来检验这一假设。

Method: 进行了迄今为止最全面的跨模板FV转移研究：4,032对样本，涵盖12个任务、6个模型（来自3个家族：Llama-3.1-8B、Gemma-2-9B、Mistral-7B-v0.3的基础版和指令调优版），每个任务8个模板。使用对数透镜和FV词汇投影分析，并进行激活修补以验证因果定位。

Result: 发现与假设相反：FV引导成功时，对数透镜仍无法在任何层解码正确答案。引导-无解码模式具有普遍性：在所有任务和模型上，引导准确率均超过对数透镜，最大差距达-0.91。仅3个任务-模型实例显示预测的解码-无引导模式（均在Mistral中）。FV词汇投影显示高引导准确率的FVs仍投影到不连贯的标记分布。FV在早期层（L2-L8）干预效果最佳，而对数透镜仅在晚期层（L28-L32）检测到正确答案。先前报告的负余弦转移相关性（r=-0.572）在大规模研究中消失。不同模型家族在引导机制上存在差异：Mistral FVs重写中间表示，而Llama/Gemma FVs产生近零变化。

Conclusion: 函数向量编码的是计算指令而非答案方向，其引导机制与对数透镜的解码能力存在根本性分离。不同模型家族采用不同的引导策略，且任务难度影响因果定位效果。这些发现挑战了关于FV引导机制的现有假设，为理解语言模型内部计算提供了新视角。
...
### [106] [Analytic Drift Resister for Non-Exemplar Continual Graph Learning](https://arxiv.org/abs/2604.02633)
*Lei Song,Shihan Guan,Youyong Kong*

Main category: cs.LG

TL;DR: ADR是一个新颖的非示例持续图学习框架，通过分层分析合并和解析分类器重建来解决特征漂移问题，实现零遗忘的类增量学习。


<details>
  <summary>Details</summary>
Motivation: 现有NECGL方法因仅保留类级原型表示而导致特征漂移，而ACL方法虽然利用预训练模型的泛化特性，但模型可塑性显著衰减。需要克服这两个挑战。

Method: 提出ADR框架：1) 使用迭代反向传播打破预训练模型冻结约束，增强模型可塑性；2) 提出HAM通过岭回归对GNN的线性变换进行分层合并，绝对抵抗特征漂移；3) 提出ACR实现理论上零遗忘的类增量学习。

Result: 在四个节点分类基准测试上的实证评估表明，ADR与现有最先进方法相比保持了强大的竞争力。

Conclusion: ADR通过结合迭代反向传播、分层分析合并和解析分类器重建，有效解决了NECGL中的特征漂移问题，同时保持了模型可塑性，实现了零遗忘的持续图学习。
...
### [107] [AXELRAM: Quantize Once, Never Dequantize](https://arxiv.org/abs/2604.02638)
*Yasushi Nishida*

Main category: cs.LG

TL;DR: AXELRAM是一种智能SRAM宏架构，可直接从量化KV缓存索引计算注意力分数而无需反量化，通过设计时固定码本和不对称路径设计实现102.4倍乘法减少，并解决了某些模型中的灾难性PPL峰值问题。


<details>
  <summary>Details</summary>
Motivation: 动机是解决KV缓存量化中的计算效率问题。传统方法需要反量化操作，而AXELRAM旨在直接从量化索引计算注意力分数，减少计算开销。同时发现某些模型存在符号模式敏感性导致的灾难性PPL峰值问题，需要解决这一稳定性问题。

Method: 方法包括：1）基于正交变换的量化将坐标分布集中到N(0,1/d)，创建设计时固定码本；2）不对称路径设计（写入时变换，读取时查表，无逆变换）；3）通过多种子评估发现符号模式敏感性现象；4）提出基于层间范数异质性的梯度无关符号模式选择方法（200候选，8校准样本，一次性）。

Result: 结果：1）实现每查询乘法减少102.4倍；2）发现Qwen2.5-3B等模型存在灾难性PPL峰值（Δ>50），而LLaMA-3.1-8B完全稳定；3）提出的符号模式选择方法消除了灾难性峰值，无需额外硬件成本；4）所有源代码已开源。

Conclusion: 结论：AXELRAM架构成功实现了直接从量化KV缓存索引计算注意力分数，显著减少计算开销。同时揭示了KV缓存量化中的符号模式敏感性现象，并提供了有效的解决方案，为高效稳定的注意力计算提供了新方法。
...
### [108] [Conditional Sampling via Wasserstein Autoencoders and Triangular Transport](https://arxiv.org/abs/2604.02644)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够同时利用被条件变量和条件变量低维结构的条件模拟框架，改进现有方法在条件分布支撑集真正低维时的性能。

Method: 修改Wasserstein自编码器，采用(块)三角解码器结构，对潜在变量施加独立性假设，建立与条件最优传输问题的理论联系，提出三种架构变体。

Result: 不同CWAE变体相比低秩集成卡尔曼滤波器(LREnKF)显著降低了近似误差，特别是在条件测度支撑集真正低维的问题中表现优异。

Conclusion: CWAEs提供了一个有效的条件模拟框架，能够利用低维结构，在条件分布支撑集低维时相比传统方法有显著优势，并建立了与条件最优传输的理论联系。
...
### [109] [Generalization Limits of Reinforcement Learning Alignment](https://arxiv.org/abs/2604.02652)
*Haruhi Shida,Koo Imai,Keigo Kansa*

Main category: cs.LG

TL;DR: 通过组合多种攻击技术形成"复合越狱"，针对OpenAI GPT模型的安全对齐机制进行攻击，攻击成功率从14.3%提升至71.4%，证明安全训练泛化能力弱于模型能力


<details>
  <summary>Details</summary>
Motivation: 现有理论分析表明基于强化学习的对齐训练可能只是重新分配现有能力的利用概率，而非获得新能力。本研究旨在通过实证验证安全对齐的泛化失败问题，特别是当多种攻击技术组合时，安全机制可能被绕过

Method: 提出"复合越狱"攻击方法，针对OpenAI gpt-oss-20b模型，组合多种已被单独防御的攻击技术，通过饱和指令层次维护过程来利用对齐的泛化失败

Result: 攻击成功率从单一方法的14.3%显著提升至复合攻击的71.4%，为安全训练泛化能力弱于模型能力的假设提供了实证证据

Conclusion: 安全对齐训练无法像模型能力那样广泛泛化，需要使用复合攻击场景进行多方面的安全评估，当前的安全机制在面对组合攻击时存在显著脆弱性
...
### [110] [Product-Stability: Provable Convergence for Gradient Descent on the Edge of Stability](https://arxiv.org/abs/2604.02653)
*Eric Gan*

Main category: cs.LG

TL;DR: 该论文研究了损失函数的乘积稳定性特性，证明了对于具有乘积稳定极小值的损失函数，即使在边缘稳定性(EoS)区域训练，梯度下降也能收敛到局部极小值，这推广了先前结果并适用于更广泛的损失函数类别。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习训练常在边缘稳定性(EoS)区域进行，此时损失函数的锐度超过了经典收敛分析适用的阈值。现有EoS理论解释要么依赖严格假设，要么仅关注特定平方损失类型目标，需要更通用的理论框架来解释更广泛损失函数在EoS区域的稳定训练。

Method: 引入并研究损失函数的乘积稳定性结构特性，证明对于具有乘积稳定极小值的损失函数，梯度下降应用于形式为$(x,y) \mapsto l(xy)$的目标函数时，即使在EoS区域也能收敛到局部极小值。使用分岔图分析训练动态，解释稳定振荡的出现，并精确量化收敛时的锐度。

Result: 该框架显著推广了先前结果，适用于包括二元交叉熵在内的广泛损失函数类别。通过分岔图表征了训练动态，解释了稳定振荡的出现机制，并精确量化了收敛时的锐度，为更广泛损失函数在EoS区域的稳定训练提供了原理性解释。

Conclusion: 乘积稳定性为理解损失函数在边缘稳定性区域的训练行为提供了新的理论框架，能够解释更广泛损失函数在EoS区域的稳定训练动态，包括收敛性、振荡现象和锐度量化，推进了对现代深度学习训练机制的理论理解。
...
### [111] [Finding Belief Geometries with Sparse Autoencoders](https://arxiv.org/abs/2604.02685)
*Matthew Levinson*

Main category: cs.LG

TL;DR: 提出了一种从transformer表示中发现候选单纯形结构子空间的流程，在Gemma-2-9B中识别出13个具有单纯形几何特征的优先级簇，其中5个通过了重心的预测测试，为大型语言模型中存在类似信念状态的几何表示提供了初步证据。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在自然文本训练中是否发展出类似隐藏马尔可夫模型中观察到的信念状态几何表示（单纯形结构），这是机制可解释性的核心问题。

Method: 提出一个发现候选单纯形结构子空间的流程：结合稀疏自编码器（SAEs）、SAE特征的k-子空间聚类，以及使用AANet进行单纯形拟合。在已知信念状态几何的多部分隐藏马尔可夫模型上验证流程，然后应用于Gemma-2-9B模型。

Result: 在Gemma-2-9B中识别出13个优先级簇（K≥3）表现出候选单纯形几何。通过重心预测测试，3个簇在近顶点样本上表现出显著优势（Wilcoxon p<10^{-14}），4个在单纯形内部样本上表现良好。共5个不同的真实簇至少通过一个分割测试，而零簇均未通过。簇768_596还获得了最高的因果导向分数。

Conclusion: 这些发现为Gemma-2-9B表示空间中存在真正的信念状几何提供了初步证据，但需要结构化评估来确认这一解释。研究强调了区分真实信念状态编码与平铺伪影的重要性，并提出了重心预测作为主要判别测试。
...
### [112] [Beyond Semantic Manipulation: Token-Space Attacks on Reward Models](https://arxiv.org/abs/2604.02686)
*Yuheng Zhang,Mingyue Huo,Minghao Zhu,Mengxue Zhang,Nan Jiang*

Main category: cs.LG

TL;DR: TOMPA是一种直接在词元空间进行对抗优化的攻击框架，通过绕过策略与奖励模型之间的解码-重新词元化接口，利用黑盒标量反馈发现能获得极高奖励的非语言词元模式，揭示了当前RLHF流程中的关键漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型攻击主要在语义空间进行，构建人类可读的对抗输出以利用奖励模型偏差。本研究旨在探索一种根本不同的范式：直接在词元空间进行对抗优化，以揭示奖励模型在语义空间之外的系统性可被利用性。

Method: 提出Token Mapping Perturbation Attack (TOMPA)框架，绕过策略与奖励模型之间的标准解码-重新词元化接口，使攻击策略能够在原始词元序列而非连贯自然语言上进行优化。仅使用黑盒标量反馈，自动发现能引发极高奖励的非语言词元模式。

Result: 针对Skywork-Reward-V2-Llama-3.1-8B模型，TOMPA几乎将GPT-5参考答案的奖励翻倍，并在98.0%的提示上优于GPT-5。尽管获得高分，生成的输出退化为无意义文本，表明奖励模型可以在语义体系之外被系统性利用。

Conclusion: TOMPA揭示了当前RLHF流程中的关键漏洞：奖励模型不仅容易受到语义空间攻击，还能在词元空间被系统性利用，生成无意义但获得高奖励的文本，这对基于奖励模型的RLHF系统的安全性提出了重要挑战。
...
### [113] [Adaptive Semantic Communication for Wireless Image Transmission Leveraging Mixture-of-Experts Mechanism](https://arxiv.org/abs/2604.02691)
*Haowen Wan,Qianqian Yang*

Main category: cs.LG

TL;DR: 提出基于自适应MoE Swin Transformer的多阶段端到端图像语义通信系统，通过联合评估CSI和图像语义内容的动态专家门控机制，实现更高效的MIMO信道传输。
...
Method: 构建基于自适应MoE Swin Transformer块的多阶段端到端图像语义通信系统，引入动态专家门控机制，联合评估实时CSI和输入图像块的语义内容来计算自适应路由概率，仅基于联合条件激活专家子集。

Result: 仿真结果表明，相比现有方法在保持传输效率的同时，重建质量有显著提升。

Conclusion: 提出的多驱动路由方法打破了传统自适应方法的刚性耦合，克服了单驱动路由的瓶颈，实现了更高效的MIMO信道语义通信。
...
### [114] [FluxMoE: Decoupling Expert Residency for High-Performance MoE Serving](https://arxiv.org/abs/2604.02715)
*Qingxiu Liu,Cyril Y. He,Hanser Jiang,Zion Wang,Alan Zhao,Patrick P. C. Lee*

Main category: cs.LG

TL;DR: FluxMoE是一个新的MoE推理系统，通过专家分页抽象将专家权重从GPU常驻内存中解耦，按需加载和驱逐，优先分配内存给KV缓存，在内存受限环境下实现高达3倍的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: MoE模型参数规模快速增长导致推理时存在根本性低效：大多数专家权重在GPU内存中闲置，同时与性能关键运行时状态（如KV缓存）竞争内存。由于KV缓存容量直接决定服务吞吐量，这种不匹配导致内存利用不足和性能下降。
...
Conclusion: FluxMoE通过解耦专家参数与GPU常驻内存，解决了MoE推理中的内存效率瓶颈，为大规模MoE模型部署提供了高效的内存管理方案。
...
### [115] [Generative Frontiers: Why Evaluation Matters for Diffusion Language Models](https://arxiv.org/abs/2604.02718)
*Patrick Pynadath,Jiaxin Shi,Ruqi Zhang*

Main category: cs.LG

TL;DR: 该技术笔记讨论了扩散语言模型评估方法的局限性，提出了基于KL散度分解的生成前沿作为更可靠的评估指标
...
Method: 分析当前评估方法的局限性，提出将生成困惑度和熵作为KL散度的两个组成部分，建议使用生成前沿作为评估模型生成质量的原则性方法

Result: 揭示了生成困惑度对熵的敏感性，提出了生成前沿作为更可靠的评估指标，并在GPT-2小规模（1.5亿参数）尺度上提供了实证观察

Conclusion: 需要采用更原则性的评估方法来可靠比较扩散语言模型，生成前沿提供了更好的生成质量评估框架
...
### [116] [Understanding Latent Diffusability via Fisher Geometry](https://arxiv.org/abs/2604.02751)
*Jing Gu,Morteza Mardani,Wonjun Lee,Dongmian Zou,Gilad Lerman*

Main category: cs.LG

TL;DR: 该论文提出一个理论框架，通过MMSE变化率量化潜在空间扩散性，将其分解为Fisher信息和Fisher信息率，识别潜在几何失真对扩散模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在潜在空间（如VAEs）训练时性能下降，但其根本原因尚不明确。需要量化潜在空间的可扩散性，并理解几何失真如何影响扩散过程。

Method: 通过最小均方误差变化率量化潜在空间扩散性，将其分解为Fisher信息和Fisher信息率。分析编码器局部几何特性对FIR的影响，将潜在几何失真分解为三个可测量惩罚项：维度压缩、切向失真和曲率注入。

Result: 建立了跨空间FIR保持的理论条件，确保扩散性维持。在不同自编码架构上的实验验证了该框架，并证明FI和FIR指标可作为识别和缓解潜在扩散失败的稳健诊断工具。

Conclusion: 该研究提供了一个理论框架，通过量化潜在空间几何失真对扩散过程的影响，解释了潜在空间扩散模型性能下降的原因，并为改进潜在空间扩散模型提供了诊断工具。
...
### [117] [Towards Realistic Class-Incremental Learning with Free-Flow Increments](https://arxiv.org/abs/2604.02765)
*Zhiming Xu,Baile Xu,Jian Zhao,Furao Shen,Suorong Yang*

Main category: cs.LG

TL;DR: 该论文提出了自由流类增量学习(FFCIL)新范式，针对现实场景中类别数量高度变化的增量学习问题，提出类平均目标(CWM)和多种方法调整策略，显著提升了现有CIL方法在自由流场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有类增量学习(CIL)研究通常在预定义、等大小任务的简化场景下评估，而现实场景中数据流通常包含高度变化的类别数量。这种自由流场景会使现有CIL方法变得脆弱并导致性能显著下降，因此需要研究更实用的自由流类增量学习(FFCIL)问题。

Method: 提出了模型无关的FFCIL框架：1) 类平均目标(CWM)，用均匀聚合的类条件监督替代样本频率加权损失，稳定自由流类别增量下的学习信号；2) 针对代表性CIL范式的调整策略：约束蒸馏到回放数据、归一化对比和知识转移损失的尺度、引入动态干预权重对齐(DIWA)防止小类别增量导致的统计不稳定和过度调整。

Result: 实验证实各种CIL基线方法在FFCIL场景下均出现明显性能下降，而论文提出的策略在所有基线方法上均带来一致性能提升，验证了所提方法的有效性。

Conclusion: 论文形式化了自由流类增量学习(FFCIL)这一更现实的增量学习场景，提出的类平均目标和多种调整策略能够有效提升现有CIL方法在自由流场景下的鲁棒性，为实际应用中的增量学习系统提供了实用解决方案。
...
### [118] [Extracting Money Laundering Transactions from Quasi-Temporal Graph Representation](https://arxiv.org/abs/2604.02899)
*Haseeb Tariq,Marwan Hassani*

Main category: cs.LG

TL;DR: ExSTraQt：一种基于准时序图表示的监督学习框架，用于检测金融交易中的洗钱活动，相比现有方法在性能和可扩展性方面均有提升。


<details>
  <summary>Details</summary>
Motivation: 传统反洗钱方法主要依赖预定义的风险规则，导致调查资源密集且误报率高。金融机构需要更复杂的机制来改进现有系统，以控制运营成本并提高检测效率。

Method: 提出ExSTraQt框架，采用准时序图表示提取可疑交易，是一种先进的监督学习方法。框架设计简单、参数少，计算和内存需求具有可扩展性。

Result: 在真实数据集和合成金融交易数据集上评估，相比最先进的AML检测模型，F1分数均有提升：真实数据集提升达1%，合成数据集提升超过8%。

Conclusion: ExSTraQt框架在检测洗钱交易方面表现出色，设计简单、可扩展性强，能够无缝补充银行现有的AML检测系统。
...
### [119] [Efficient Logistic Regression with Mixture of Sigmoids](https://arxiv.org/abs/2604.02920)
*Federico Di Gennaro,Saptarshi Chakraborty,Nikita Zhivotovskiy*

Main category: cs.LG

TL;DR: 指数权重算法在线性可分情况下，通过各向同性高斯先验实现O(d log(Bn))遗憾界，计算复杂度从O(B¹⁸n³⁷)大幅降至O(B³n⁵)，并在大B极限下收敛到截断高斯分布，与硬间隔SVM方向对齐。


<details>
  <summary>Details</summary>
Motivation: 研究指数权重算法在线性逻辑回归中的计算效率问题，解决先前工作计算复杂度极高（O(B¹⁸n³⁷)）的瓶颈，同时探索在线性可分情况下算法的几何适应性。

Method: 采用各向同性高斯先验的指数权重算法，分析其在线性可分大B极限下的后验分布收敛行为，证明其收敛到版本锥上的截断标准高斯分布，并建立与硬间隔SVM的几何联系。

Result: 将计算复杂度从O(B¹⁸n³⁷)大幅降低到O(B³n⁵)，同时保持O(d log(Bn))的近似最优遗憾界。在线性可分情况下，当B超过间隔依赖阈值时，遗憾界与B无关，仅随逆间隔对数增长。

Conclusion: 指数权重算法可以在保持近似最优遗憾界的同时实现计算可行性，并在在线分类中展现出几何适应性，特别是在线性可分情况下能自适应地收敛到硬间隔SVM方向。
...
### [120] [Explainable Machine Learning Reveals 12-Fold Ucp1 Upregulation and Thermogenic Reprogramming in Female Mouse White Adipose Tissue After 37 Days of Microgravity: First AI/ML Analysis of NASA OSD-970](https://arxiv.org/abs/2604.02942)
*Md. Rashadul Islam*
...
<details>
  <summary>Details</summary>
Motivation: 微重力对哺乳动物生理产生深远代谢适应，但雌性白色脂肪组织产热的分子机制尚不清楚。本研究旨在利用机器学习重新分析NASA新发布的太空生物学数据集，揭示微重力对雌性脂肪组织产热通路的影响。
...
Result: 微重力暴露的白色脂肪组织中Ucp1基因表达显著上调12.21倍，产热通路平均上调3.24倍。最佳模型（随机森林，前20特征）AUC=0.922，准确率=0.812，F1=0.824。SHAP分析显示Ucp1是顶级预测特征，Angpt2、Irs2、Jun和Klf家族转录因子是主要分类器。PCA显示飞行与地面样本明显分离，PC1解释69.1%方差。

Conclusion: 微重力诱导雌性白色脂肪组织快速产热重编程，作为对微重力环境的代偿反应。研究展示了可解释AI在重新分析NASA太空生物学数据集中的强大能力，对长期太空任务中女性宇航员健康以及地球上的肥胖和代谢疾病研究具有直接意义。
...
TL;DR: 提出一种基于扩散模型的倾斜分布采样方法，通过插件估计器实现最小最大最优性，并建立Wasserstein距离和TV精度理论保证


<details>
  <summary>Details</summary>
Motivation: 在金融、天气气候建模等领域，需要从满足特定矩约束的倾斜分布中生成样本。现有方法缺乏理论保证，需要开发具有理论保证的扩散采样方法

Method: 提出插件估计器方法，从原始分布中采样，通过参数θ控制倾斜程度。建立Wasserstein距离理论界，分析插件估计器分布与真实分布的关系，并在一定假设下证明扩散过程的TV精度

Result: 证明插件估计器具有最小最大最优性，建立了n和θ的函数形式的Wasserstein距离理论界，识别了输出与真实分布接近的机制，并在假设下证明了扩散采样的TV精度

Conclusion: 该方法为从倾斜分布中生成样本提供了理论保证的扩散采样框架，在金融、天气气候建模等领域有广泛应用前景，并通过大量模拟验证了理论结果
...
TL;DR: Self-Guide：为语言智能体提出的自生成内部奖励方法，支持推理时引导和训练时监督，通过策略与内部奖励的协同进化提升性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体通过与环境的交互学习，但长时程训练面临稀疏和延迟奖励的根本瓶颈。现有方法通常通过事后信用分配或外部奖励模型解决，这些方法在推理时提供有限指导，且往往将奖励改进与策略改进分离。

Method: 提出Self-Guide方法，智能体使用自生成的内部奖励作为短期自引导信号在推理时引导下一步行动，并将相同信号转换为步级内部奖励用于训练时更密集的策略优化。这创建了协同进化循环：更好的策略产生更好的引导，更好的引导作为内部奖励进一步改进策略。

Result: 在三个智能体基准测试中，推理时自引导已带来明显收益，而使用GRPO联合进化策略和内部奖励相比仅使用环境奖励训练的基线带来进一步8%的改进。

Conclusion: 语言智能体不仅可以通过收集更多经验改进，还可以通过在行动和学习过程中学习生成和优化自己的内部奖励来提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：纯基于特权教师的自蒸馏会导致信息泄露和训练不稳定，而仅依赖环境反馈的RLVR只能获得稀疏信号

Method: 提出RLSD方法，结合RLVR和自蒸馏的优势：使用自蒸馏获得token级策略差异来确定细粒度更新幅度，同时继续使用RLVR从环境反馈中获取可靠的更新方向

Result: RLSD能够同时利用RLVR和OPSD的优势，实现更高的收敛上限和更好的训练稳定性

Conclusion: RLSD方法通过将自蒸馏用于确定更新幅度、RLVR用于确定更新方向，找到了自蒸馏的最佳应用场景，解决了现有方法的局限性
...
### [124] [Understanding the Role of Hallucination in Reinforcement Post-Training of Multimodal Reasoning Models](https://arxiv.org/abs/2604.03179)
...
TL;DR: 本文提出了幻觉作为线索框架，用于分析RL后训练对多模态大语言模型视觉推理能力的影响，通过引入幻觉诱导的模态特定破坏来研究模型是否真正从视觉信息中学习。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在多模态大语言模型后训练中广泛应用且报告了性能提升，但尚不清楚RL训练是否真正使模型从视觉信息中学习。需要一种分析框架来研究RL后训练对多模态推理模型的影响，特别是从模型幻觉的角度。

Method: 提出幻觉作为线索框架，引入幻觉诱导的模态特定破坏，移除或替换推导正确答案所需的关键信息，迫使模型通过幻觉进行推理。在训练和评估阶段应用这些破坏，为诊断RL训练动态和理解数据集内在特性提供独特视角。

Result: 在多个多模态推理基准测试中的广泛实验和分析表明，模型幻觉在RL训练中的作用比先前认识到的更为重要。在纯幻觉诱导设置下进行RL后训练仍能显著提升模型推理性能，在某些情况下甚至优于标准训练。

Conclusion: 这些发现挑战了关于MLLM推理训练的普遍假设，并激励开发更具模态感知的基于RL的训练设计。幻觉作为线索框架为理解RL后训练对多模态模型的影响提供了新的分析工具。
...
TL;DR: RCL是一个统一框架，将上下文学习视为优化问题，通过反思和突变机制改进上下文空间中的学习，并系统应用经典优化原语提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前上下文学习方法零散且临时，未充分探索学习中的基本问题（信用分配、过拟合、遗忘、局部最优、高方差学习信号等），需要统一框架来系统改进。

Method: 提出反思性上下文学习(RCL)框架：反思机制将轨迹和当前上下文转换为类似梯度的方向性更新信号，突变机制应用该信号改进上下文空间中的未来行为。将现有方法重新表述为此共享学习问题实例，并系统引入批处理、改进信用分配信号、辅助损失、失败回放、分组rollouts等经典优化原语。

Result: 在AppWorld、BrowseComp+和RewardBench2上，这些优化原语显著提升了强基线的性能，且不同任务机制下各原语的重要性不同。分析了初始化鲁棒性、批量大小影响、采样和课程策略、优化器状态变体以及不同强度模型分配的影响。

Conclusion: 上下文学习不应被视为孤立算法集合，而应作为优化问题处理，其机制可通过可转移原则进行系统研究和改进。RCL为此提供了统一框架。
...
### [126] [Gradient Boosting within a Single Attention Layer](https://arxiv.org/abs/2604.03190)
*Saleh Sargolzaei*

Main category: cs.LG

TL;DR: 提出梯度提升注意力机制，在单层注意力中引入梯度提升思想：第二遍注意力关注第一遍的预测误差并进行门控校正，显著降低语言建模困惑度。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力通过单次softmax加权平均计算值，这种一次性估计无法自我校正错误。需要一种机制能够在注意力层内部进行误差校正。

Method: 提出梯度提升注意力：1）第一遍标准注意力计算；2）第二遍注意力关注第一遍的预测误差，使用独立学习的投影；3）通过逐维度门控进行校正。该方法将梯度提升思想映射到注意力机制中，每个注意力遍作为基础学习器，门控作为收缩参数。

Result: 在WikiText-103的10M token子集上，梯度提升注意力测试困惑度为67.9，优于标准注意力(72.2)、Twicing注意力(69.6)和参数匹配的加宽基线(69.0)。两轮注意力即可获得大部分收益。

Conclusion: 梯度提升注意力通过引入内部校正机制有效提升了注意力性能，将梯度提升原理成功应用于注意力机制设计，为注意力架构改进提供了新思路。
...
### [127] [Hierarchical Planning with Latent World Models](https://arxiv.org/abs/2604.03208)
...
<details>
  <summary>Details</summary>
Motivation: 基于学习世界模型的模型预测控制（MPC）在具身控制中表现出色，特别是在新环境中具有零样本泛化能力。然而，学习的世界模型在长时域控制中存在两个主要问题：1）预测误差的累积；2）搜索空间随规划时域呈指数增长。这些问题限制了学习世界模型在复杂、长时域任务中的应用。

Method: 提出了一种分层规划方法：1）在多时间尺度上学习潜在世界模型；2）在这些尺度上进行分层规划。这种方法作为一个模块化的规划抽象，适用于多种潜在世界模型架构和领域。通过在不同时间尺度上进行规划，实现了长时域推理，同时显著降低了推理时的规划复杂度。

Result: 1）在真实世界非贪婪机器人任务中实现了零样本控制，在仅使用最终目标规格的情况下，拾取放置任务的成功率达到70%，而单层世界模型为0%；2）在物理模拟环境中（包括推动操作和迷宫导航），分层规划实现了更高的成功率，同时规划时间计算需求减少了最多4倍。

Conclusion: 分层规划方法通过在多时间尺度上学习世界模型并进行分层规划，有效解决了学习世界模型在长时域控制中的挑战。该方法不仅提高了任务成功率，还显著降低了规划复杂度，为基于学习世界模型的模型预测控制在复杂、长时域任务中的应用提供了有效解决方案。
...
### [128] [Enhancing Robustness of Federated Learning via Server Learning](https://arxiv.org/abs/2604.03226)
*Van Sy Mai,Kushal Chakrabarti,Richard J. La,Dipankar Maity*

Main category: cs.LG

TL;DR: 提出一种结合服务器学习和客户端更新过滤的联邦学习鲁棒性增强方法，能在非IID数据和高比例恶意客户端情况下显著提升模型精度


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习在非独立同分布数据和恶意攻击下的鲁棒性问题，特别是在高比例恶意客户端情况下保护模型训练的安全性

Method: 提出启发式算法，结合服务器学习和客户端更新过滤，使用几何中位数聚合方法，服务器可使用小规模甚至合成数据集

Result: 实验证明该方法能显著提升模型精度，即使恶意客户端比例超过50%，且服务器数据集小、分布与客户端聚合数据不一定相近
...
### [129] [MLFCIL: A Multi-Level Forgetting Mitigation Framework for Federated Class-Incremental Learning in LEO Satellites](https://arxiv.org/abs/2604.02356)
*Heng Zhang,Xiaohong Deng,Sijing Duan,Wu Ouyang,KM Mahfujul,Yiqin Deng,Zhigang Chen*

Main category: cs.NI

TL;DR: MLFCIL是一个针对低地球轨道卫星星座的多层次遗忘缓解联邦类增量学习框架，通过分解灾难性遗忘的三个来源，在不同层次采用针对性策略，显著提升了准确性和遗忘缓解效果。


<details>
  <summary>Details</summary>
Motivation: 低地球轨道卫星星座在严格内存和通信约束下进行协同训练面临三大挑战：轨道动态导致的非独立同分布数据异质性、聚合过程中放大的灾难性遗忘、以及有限资源下稳定性与可塑性平衡的需求。现有联邦类增量学习方法难以有效应对这些LEO特定挑战。

Method: 提出MLFCIL多层次遗忘缓解框架：1) 类重加权损失减少本地偏差；2) 结合特征回放和原型引导漂移补偿的知识蒸馏保留跨任务知识；3) 类感知聚合缓解联邦过程中的遗忘。此外设计双粒度协调策略：轮级自适应损失平衡和步级梯度投影，增强稳定性-可塑性权衡。

Result: 在NWPU-RESISC45数据集上的实验表明，MLFCIL在准确性和遗忘缓解方面显著优于基线方法，同时引入的资源开销最小。

Conclusion: MLFCIL通过分解灾难性遗忘的三个来源并在不同层次针对性解决，有效应对了LEO卫星星座联邦类增量学习的特定挑战，为资源受限环境下的分布式增量学习提供了有效解决方案。
...
### [130] [TRACE: Traceroute-based Internet Route change Analysis with Ensemble Learning](https://arxiv.org/abs/2604.02361)
*Raul Suzuki,Rodrigo Moreira,Pedro Henrique A. Damaso de Melo,Larissa F. Rodrigues Moreira,Flávio de Oliveira Silva*

Main category: cs.NI

TL;DR: TRACE是一个仅使用traceroute延迟数据检测互联网路由不稳定的机器学习管道，通过特征工程和堆叠集成方法，在类不平衡情况下实现高F1分数


<details>
  <summary>Details</summary>
Motivation: 检测互联网路由不稳定性是一个关键但具有挑战性的任务，特别是当仅依赖端点主动测量时。传统方法需要控制平面信息，而TRACE旨在仅使用traceroute延迟数据来识别路由变化，确保独立于控制平面信息

Method: 提出一个稳健的特征工程策略，使用滚动统计和聚合上下文模式捕获时间动态。架构采用梯度提升决策树的堆叠集成，通过超参数优化的元学习器进行精炼。通过严格校准决策阈值来处理罕见路由事件固有的类别不平衡问题

Result: TRACE实现了优越的F1分数性能，显著优于传统基线模型，并在检测互联网路由变化方面表现出强大的有效性

Conclusion: TRACE是一个有效的机器学习管道，能够仅使用traceroute延迟数据检测路由不稳定性，通过创新的特征工程和集成学习方法解决了类别不平衡问题，为互联网路由监测提供了新的解决方案
...
### [131] [Evaluating Small Language Models for Front-Door Routing: A Harmonized Benchmark and Synthetic-Traffic Experiment](https://arxiv.org/abs/2604.02367)
*Warren Johnson,Charles Lee*

Main category: cs.NI

TL;DR: 小型语言模型（SLMs）已具备足够推理能力进行零边际成本、自托管的任务分类，可作为高效路由决策方案，但准确率仍需提升6-8个百分点才能达到生产可用标准。


<details>
  <summary>Details</summary>
Motivation: 现有路由方法依赖大型语言模型分类器或偏好训练的路由器，这些方案本身成本高、延迟大，且将多目标优化简化为单维质量预测。需要探索更高效的路由决策方案。

Method: 通过两项研究验证SLMs在路由决策中的可行性：研究1在统一硬件和量化条件下，对Phi-3.5-mini、Qwen2.5-1.5B和Qwen-2.5-3B进行离线基准测试；研究2进行预注册的四臂随机实验，比较Phi-4-mini、Qwen-2.5-3B、DeepSeek-V3和无路由控制组在合成流量下的表现。

Result: Qwen-2.5-3B在研究1中获得最佳精确匹配准确率（0.783）和延迟-准确率权衡；研究2中DeepSeek-V3准确率最高（0.830）但延迟超标，Qwen-2.5-3B在自托管模型中帕累托最优（0.793准确率，988ms中位延迟，零边际成本）。所有模型均未达到独立可用标准（≥0.85准确率，≤2000ms P95延迟）。

Conclusion: SLM路由的成本和延迟前提已满足，但准确率差距6-8个百分点，且正确分类是否转化为下游输出质量尚未验证，这些因素限制了其生产可行性。SLM路由决策在推理预算中可忽略不计的潜力已得到证实。
...
### [132] [Beyond Message Passing: A Semantic View of Agent Communication Protocols](https://arxiv.org/abs/2604.02369)
...
TL;DR: 论文提出了一个三层框架（通信、句法、语义）来分析18种智能体通信协议，发现当前协议设计存在不平衡：传输和句法层支持成熟，但语义层机制不足，导致语义责任被推给提示词、包装器或应用逻辑，产生技术债务。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型系统需要工具使用、多智能体协调和异构环境操作，智能体通信协议变得至关重要。当前缺乏系统性的协议分析框架，且协议设计存在不平衡，语义层支持不足导致技术债务和互操作性问题。

Method: 提出人类启发的三层框架（通信层、句法层、语义层），系统分析18种代表性协议，评估它们在可靠传输、结构化交互和意义级协调方面的支持程度，识别技术债务形式并提供协议选择指导。

Result: 分析显示当前协议设计明显不平衡：大多数协议在传输、流式处理、模式定义和生命周期管理方面支持成熟，但在澄清、上下文对齐和验证等协议级语义机制方面有限。语义责任被推给提示词、包装器或应用特定编排逻辑，产生隐藏的互操作性和维护成本。

Conclusion: 需要超越消息传递，实现共享理解的互操作、安全和语义鲁棒的智能体生态系统。论文提出了研究议程，包括识别技术债务、提供协议选择指导，以及推动语义层协议机制的发展。
...
### [133] [act: Technical report](https://arxiv.org/abs/2604.02955)
*Zoe Paraskevopoulou,Anja Petković Komel,Sophie Rain,Lefteris Lazaropoulos,Alexis Terry*

Main category: cs.PL

TL;DR: 本文档提供了act规范与验证语言的正式定义和元理论，包括语法、操作指针语义、类型系统及主要元理论结果（类型安全性）。


<details>
  <summary>Details</summary>
Motivation: 为act规范与验证语言提供完整的正式定义和理论基础，确保该语言具有严格的语义和类型安全保障。

Method: 采用形式化方法，定义语言的语法结构、操作指针语义模型，构建类型系统，并进行元理论证明。

Result: 建立了act语言的完整形式化规范，证明了类型安全性等关键元理论结果。

Conclusion: 该技术报告为act语言提供了坚实的理论基础，确保了其在规范与验证应用中的可靠性和安全性。
...
TL;DR: OmniTQA是一个成本感知的混合查询处理框架，用于处理同时包含结构化属性和自由文本字段的企业数据库，通过将LLM语义推理作为一等查询操作符集成到关系查询图中，优化执行成本。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL和表格问答系统大多假设所有查询相关信息都明确表示在结构化模式中，而实际企业数据库常包含结构化属性和自由文本字段共存的混合模式，需要系统同时推理两种类型的信息。

Method: OmniTQA将语义推理作为一等查询操作符，将LLM语义操作与经典关系操作符无缝集成到可执行的有向无环图中。采用数据感知规划扩展经典查询优化，结合原子查询分解和操作符重排序来最小化语义工作负载。采用双引擎执行架构，动态在关系数据库和LLM模块间路由任务，使用操作符感知批处理实现高效扩展。

Result: 在多样化的结构化和半结构化表格问答基准测试中，OmniTQA在准确性和成本效率方面持续优于现有的符号、语义和混合基线方法。对于复杂查询、大型表格和多关系模式，这些优势尤为显著。

Conclusion: OmniTQA成功解决了混合模式数据库查询处理的挑战，通过将LLM语义推理集成到查询处理管道中，并采用成本感知优化策略，实现了在准确性和效率方面的显著提升。
...
TL;DR: ReCAP框架让关系型DBMS能够通过显式指定早期过滤机会，有效处理属性图上的正则表达式路径查询，性能提升高达400,000倍


<details>
  <summary>Details</summary>
Motivation: 当前图数据库和关系数据库在处理结合正则表达式和属性约束的通用路径查询时，无法有效进行早期过滤以消除中间结果，导致查询效率低下

Method: 提出ReCAP抽象框架，只需实现NFA风格的状态转移函数和少量类似用户定义聚合的函数，即可为任何类型的属性约束实现早期过滤，并将这些过滤机会显式指定给关系优化器

Result: 使用ReCAP后，标准关系数据库DuckDB能够将属性约束深度推入查询计划，在多种查询和输入图上，性能比现有最佳图数据库和关系数据库提升高达400,000倍

Conclusion: 通过ReCAP框架为关系优化器提供"少量帮助"，可以显著提升属性图上通用路径查询的性能，为复杂约束的早期过滤提供了简洁的实现方案
...
TL;DR: HoldUp提出了一种具有整体数据理解能力的语义数据处理方法，通过联合处理记录并利用跨记录关系来准确解释数据上下文中的任务，解决了现有语义算子因缺乏整体数据理解而导致的准确性问题。


<details>
  <summary>Details</summary>
Motivation: 现有数据系统中的语义算子存在关键缺陷：缺乏整体数据理解。这些算子通常独立处理每个数据记录，不考虑数据上下文，仅依赖LLM对用户提供任务的与数据集无关的解释。然而自然语言不精确，只有在数据集的上下文中正确解释任务才能准确执行。例如，对于典型的语义映射任务（分类和评分），逐行处理的标准方法在多种数据集中产生不准确结果。

Method: HoldUp采用联合处理记录的方法，利用跨记录关系在数据上下文中正确解释任务。为解决LLM数据理解悖论（需要大量代表性数据子集提供上下文，但长输入会导致质量下降），开发了一种新颖的聚类算法，通过审慎使用LLM识别数据集中的潜在结构，灵感来自bagging。以此为基础，开发了基于聚类的分类和评分方法。

Result: 在15个真实世界数据集上的实验表明，HoldUp始终优于现有解决方案，分类任务准确率提高达33%，评分和聚类任务准确率提高达30%。

Conclusion: HoldUp通过实现整体数据理解，解决了现有语义算子的关键限制，显著提高了语义数据处理的准确性。该方法通过新颖的聚类算法解决了LLM数据理解悖论，为语义数据操作提供了更可靠的解决方案。
...
TL;DR: 对8种距离比较操作算法在10个数据集和多种硬件配置下的综合基准测试表明，这些方法虽然能加速索引构建和数据更新，但性能高度不稳定，不适合生产部署。


<details>
  <summary>Details</summary>
Motivation: 距离比较操作是向量相似性搜索的关键性能瓶颈，近期避免全维度距离计算的DCO方法承诺显著加速，但其在生产向量数据库系统中的适用性仍不明确。

Method: 对8种DCO算法在10个数据集（最多1亿向量，12,288维度）和多样化硬件配置（带/不带SIMD的CPU，GPU）上进行全面基准测试。

Result: DCO方法并非万能解决方案：其效率对数据维度高度敏感，在分布外查询下性能下降，在不同硬件上表现不稳定。但能加速索引构建和数据更新。性能不稳定可能导致比全维度扫描更慢。

Conclusion: 尽管DCO方法具有加速索引构建和数据更新的优点，但其不稳定的性能表现（可能比全维度扫描更慢）表明近期DCO算法进展尚未准备好用于生产部署。
...
<details>
  <summary>Details</summary>
Motivation: 多向量相似性搜索在细粒度语义检索中至关重要，但现有方法基于单向量索引的过滤-精炼框架存在固有困境：激进过滤牺牲召回率，保守过滤导致计算成本过高

Method: 提出MV-HNSW，首个为多向量数据设计的原生层次图索引，包含：1)满足对称性、基数鲁棒性和查询一致性的边权重函数；2)加速多向量相似度计算算法；3)动态发现拓扑断开但相关候选的增强搜索策略

Result: 在7个真实数据集上的实验表明，MV-HNSW达到最先进的搜索性能，保持超过90%的召回率，同时将搜索延迟降低高达14.0倍

Conclusion: MV-HNSW通过原生多向量索引设计解决了现有方法的固有困境，为多向量相似性搜索提供了高效、准确的解决方案
...
TL;DR: 第二届LLM+Graph研讨会聚焦大语言模型与图结构数据融合的研究前沿，探讨算法与系统创新


<details>
  <summary>Details</summary>
Motivation: LLM与图结构数据的融合已成为快速发展的研究前沿，受到学术界和工业界的广泛关注。随着图数据管理和图机器学习在实际应用中的重要性日益增长，需要探索如何有效结合LLM的能力来处理图结构数据，解决实际应用中的挑战。

Method: 通过举办第二届LLM+Graph研讨会（与VLDB 2025会议联合举办），汇集研究人员分享最新成果。研讨会聚焦于推进连接LLM、图数据管理和图机器学习的算法与系统，促进跨领域交流与合作。

Result: 研讨会成功展示了该领域的关键研究方向、挑战和创新解决方案。报告突出了LLM与图数据融合的最新进展，包括算法设计、系统实现和实际应用案例，为未来研究提供了重要参考。

Conclusion: LLM与图结构数据的融合是一个充满活力的研究领域，具有重要的理论和实践意义。研讨会为研究人员提供了交流平台，推动了该领域的发展，并为解决实际应用中的复杂问题提供了新的思路和方法。
